...
<?php
			...
			$sql = "select openaccount.*, openaccount_desc.name as name, openaccount_desc.detail, openaccount_desc.name as name2, openaccount_desc.detail2  from ".TB_OPENACCOUNT." as openaccount , ".TB_OPENACCOUNT_DESC." as openaccount_desc ";
			$sql .= " where ";
		...
		if($G_DB_CONNECT->affected_rows > 0){
			...
			while($data = $G_DB_CONNECT->fetch_array($rows)){
					...
					$openaccount_name2 =$data['name2'];
	
					$openaccount_detail2 = displayHTML($data['detail2']);
...
<!-- FAQ Icon -->
<a href="../sc/pas_software.php"><img src="../images/faq.svg" alt="" class="faq-icon"></a>
  ...
  <div class="container account-online">
    <div class="row">
        ...
        <div class="col-lg-12 online-content">
          <div class="back-to-page"><a href="../sc/pas_reg_ac.php">返回上一頁</a></div>
...
?>
            <div class="online-manual"><?php echo $title ; ?>
              <p>
                ...
                <span class="manual-text">点击此处下载</span>
...
?>		
          <br><br>
            ...
            <h4>  <?php echo $openaccount_name2; ?></h4>
              <?php echo $openaccount_detail2; ?>


<?php
	$sql = "select * from ".TB_OPENACCOUNT_PHOTO07;
	$sql .= " where openaccount_id='".ID."' and disabled=0";
	$sql .= " order by sort_order asc ";
	$rows = $G_DB_CONNECT->query($sql);
	if($G_DB_CONNECT->affected_rows > 0){

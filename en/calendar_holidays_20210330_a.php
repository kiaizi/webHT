  ...
  <div class="contaniner-fluid">
    <div class="title-bg">
      <div class="col-lg-12 ctm-width">
        ...
        <span class="other-title">Exchange Holidays</span></div>
    </div>
  </div>
...
<?php
			...
			$sql = "select YEAR(trading_hoilday.display_date) as year,trading_hoilday.*, trading_hoilday_desc.name as name  from ".TB_TRADING_HOILDAY." as trading_hoilday , ".TB_TRADING_HOILDAY_DESC." as trading_hoilday_desc ";
			$sql .= " where ";
...
?>	
	...
	<?php
$download_name= getLangName(TB_WEBPAGE,"name",47,CURRENT_LANG);

	$img = '';
	$sql = "select img from ".TB_WEBPAGE_PHOTO;
	$sql .= " where  ";
	$sql .= " disabled='0' ";
	$sql .= " and webpage_id=47 ";
	//$sql .= " and language_id='".CURRENT_LANG."' ";
	 $sql .= " order by sort_order asc  limit 0,1 ";
	$rowsp = $G_DB_CONNECT->query($sql);
	if($G_DB_CONNECT->affected_rows > 0){	
    ...
    <div class="container cf-notes">
      <div class="row">
        <div class="col-lg-12 pb-5">
          <?php echo webpageContent(22,CURRENT_LANG); ?>
        </div>
      </div>
    </div>
...
{ 
  ...
  var url2="getitem.php?p=h&date=" + d ;
...
function GetFile(str, a)
{
  ...
  /*
  var xmlhttp = GetXmlHttpObject();
  var url="getdate.php?p=h&date=" + str ;
    ...
    <thead>
      <tr>
        ...
        <div class="select-dropdown" style="border:0;">
	        <select id="c-months">
            {{ for (i = 1; i < 13; i++) { }}
		        <option value="{{: [i] }}" {{ if (month == [i-1]) {  }} selected {{ } }}>{{: [i] }}月</option>
            {{ } }}
          </select>
        </div>
      ...
      <tr>
        ...
        <td class="calendar-day {{: dayclass }} {{: thedate.toDateCssClass() }} {{: daycss[i] }} js-cal-option" data-date="{{: thedate.toISOString() }}" onclick="GetFile('{{: thedate.toDateCssClass() }}', {{: [j+1] }});"> 
          ...
          <div class="date" id="day_ck" onload="remark_colors('{{: thedate.toDateCssClass() }}',{{: [j+1] }}, {{: [i+1] }})"> 
            ...
            <div class="color_box_m" id="clr{{: thedate.toDateCssClass() }}" style="left:0; right:0;">
            </div>

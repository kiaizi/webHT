...
require_once('rc_paginator.class.php');
...
class rc_paginator_Output extends rc_paginator
{
	 ...
	 *	@var String
	 */
	var $linkTmpl = '<li><a href="HREF" class="paging_link CURRENT" title="TITLE" page="PAGE">DESCRIBE</a></li>';
	...
	{
		$this->linkTmpl = '<li><a href="HREF" class="paging_link CURRENT" title="TITLE" page="PAGE">DESCRIBE</a></li>';
	}
	...
	function getLinkNumberedLinks($seperator=' - ', $cntLinks=2)
	{
		...
		if ((count($numbers) == 1) && ($numbers[0] == $this->actual)) {
			...
			$url_numbers = $this->_createURL($url, $no, $no,"current");
			//$url_numbers = $numbers[0];
		}else {
			$cnt = 1;
			foreach ($numbers as $no) {
				...
				if ($this->actual != $no) {
					...
					//$tmp = $no;
					$tmp  = $this->_createURL($url, $no, $no,"current");
				}

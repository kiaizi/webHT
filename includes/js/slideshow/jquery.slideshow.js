...
(function($){
	...
	$.fn.slideShow = function(options) {
		...
		this.defaults = {
			...
			interval: 		5,		// interval, autoplay, set to false for no auto-play, in seconds
		...
		this.init = function() {
			...
			//alert(this.current + this.find('.navigation .page:eq(' + this.current + ')').html());
			this.find('.navigation .page:eq(' + this.current + ')').addClass('current');
			this.find('.page').click(function() {
				...
				if (!(index = parseInt($(this).html()-1))) {
					index = $(this).parents('.navigation').find('.page').index($(this));
				}
				//index = $(this).parents('.navigation').find('.page').index($(this));
		...
		this.gotoSlide = function(index, noanimation) {
			...
			oldFinished = function () {
				...
				slideShow.elm.find('.navigation .page:eq(' + slideShow.current + ')').addClass('current');
			...
			newFinished = function() {
				...
				slideShow.elm.find('.navigation .page:not(:eq(' + slideShow.current + '))').removeClass('current');
				if(slideShow.current == -1){
					slideShow.elm.find('.navigation .page:eq(0)').addClass('current');
				}
			...
			if (noanimation) {
				...
				if (typeof(this.options.transition.mode) == 'function') {
					...
					switch(this.options.transition.mode) {
						default:
						case 'fade':
							//$('#slideShow_caption').html(newSlide.attr('title'));

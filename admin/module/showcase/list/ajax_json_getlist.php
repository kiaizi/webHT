...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and showcase.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_SHOWCASE,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_SHOWCASE,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_SHOWCASE,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
//	$return_data['table_list_content'] .= '<th width="100"><a href="#" class="orderby_title" orderby="display_date"><span>'.TITLE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select showcase.*,showcase_desc.name as name,showcase_desc.detail3 from ".TB_SHOWCASE." as showcase, ".TB_SHOWCASE_DESC." as showcase_desc ";
	$sql .= " where ";
	$sql .= " showcase.id = showcase_desc.showcase_id ";
	$sql .= " and showcase.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and showcase_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by showcase.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_SHOWCASE_PHOTO," showcase_id='".$data['id']."' ","thumb",200,100);

<?php 
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and home_promotion2.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_HOME_PROMOTION2,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_HOME_PROMOTION2,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_HOME_PROMOTION2,$record_id,true,'');
		}
		generateHTACCESS();
	}
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select home_promotion2.*,home_promotion2_desc.name as name from ".TB_HOME_PROMOTION2." as home_promotion2, ".TB_HOME_PROMOTION2_DESC." as home_promotion2_desc ";
	$sql .= " where ";
	$sql .= " home_promotion2.id = home_promotion2_desc.home_promotion2_id ";
	$sql .= " and home_promotion2.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and home_promotion2_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by home_promotion2.id  ";
...
/*
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_HOME_PROMOTION2_PHOTO," home_promotion2_id='".$data['id']."' ","thumb",200,1000);

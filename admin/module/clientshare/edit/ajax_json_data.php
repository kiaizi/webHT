...
if( $_SESSION['already_login']){
	...
	$update_data['website'] = getRequestVar('website','');
...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select clientshare.* from ".TB_CLIENTSHARE." as clientshare, ".TB_CLIENTSHARE_DESC." as clientshare_desc ";
			$sql .= " where ";
			$sql .= " clientshare_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and clientshare_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and clientshare_desc.clientshare_id=clientshare.id ";
			$sql .= " and clientshare.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " clientshare.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_CLIENTSHARE, $update_data); 
			$update_data_lang['clientshare_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_CLIENTSHARE, $update_data_lang);
...
/*
			...
			$sql .= " and clientshare_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and clientshare_desc.clientshare_id=clientshare.id ";
			$sql .= " and clientshare.id<>'".$update_data['id']."'  ";
			$sql .= " and clientshare.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_CLIENTSHARE, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['clientshare_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_CLIENTSHARE, $update_data_lang, "  clientshare_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_CLIENTSHARE,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_CLIENTSHARE," id>0 ");

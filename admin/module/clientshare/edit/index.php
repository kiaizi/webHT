<?php 
	...
	$sql = "select * from ".TB_CLIENTSHARE." as clientshare ";
	$sql .= " where ";
	$sql .= " id='".ID."' ";
...
// upload file
/////////////////////////////////////////////
$arr_clientshare_photo_language_id = getRequestVar("clientshare_photo_language_id",'');
$arr_clientshare_photo_sort_order = getRequestVar("clientshare_photo_sort_order",'');
$arr_clientshare_photo_caption = getRequestVar("clientshare_photo_caption",'');
$arr_clientshare_photo_disabled = getRequestVar("clientshare_photo_disabled",'');
$arr_clientshare_photo_img_old_path = getRequestVar("clientshare_photo_img_old_path",'');
$arr_clientshare_photo_thumb_old_path = getRequestVar("clientshare_photo_thumb_old_path",'');
if($arr_clientshare_photo_disabled != '' && ID > 0){

	$clientshare_id = ID;
	//$clientshare_code = getDataName(TB_CLIENTSHARE,"code",$clientshare_id);
	$sql = "delete from ".TB_CLIENTSHARE_PHOTO." where clientshare_id='".$clientshare_id."'";
	$G_DB_CONNECT->query($sql);

	for ($i=0; $i<count($arr_clientshare_photo_disabled); $i++) {




		$clientshare_photo_img_old_path = $arr_clientshare_photo_img_old_path[$i];
		$clientshare_photo_img = $clientshare_photo_img_old_path;

		$dir = "upload/images/clientshare/";
		$file_name = "clientshare_photo_img";
		////////////////////////////////////////////////
		$tempFile = $_FILES[$file_name]['tmp_name'][$i];
		if($tempFile != ''){
			$targetPath = CURRENT_DOCUMENT_ROOT . $dir ;
			$ext = get_extension($_FILES[$file_name]['name'][$i]);
			//$new_file_name = $clientshare_code."_".date("YmdHis")."_".getLastID(TB_CLIENTSHARE_PHOTO).$ext;
			$new_file_name = date("YmdHis")."_".getLastID(TB_CLIENTSHARE_PHOTO).$ext;
  			$targetFile =  str_replace('//','/',$targetPath) .$new_file_name ;
			move_uploaded_file($tempFile,$targetFile);
			$targetFile =$dir.$new_file_name;
			////////////////////////////////////////////////

			$clientshare_photo_img  = $targetFile;
		...
		if($clientshare_photo_img != '' ){
			////////////////////////////////////////////////
			$update_clientshare_photo_data = array();
			$update_clientshare_photo_data['language_id'] = $arr_clientshare_photo_language_id[$i];
			$update_clientshare_photo_data['img'] = $clientshare_photo_img;
			$update_clientshare_photo_data['thumb'] = $clientshare_photo_thumb;
			$update_clientshare_photo_data['caption'] = $arr_clientshare_photo_caption[$i];
			$update_clientshare_photo_data['sort_order'] = $arr_clientshare_photo_sort_order[$i];
			$update_clientshare_photo_data['disabled'] = $arr_clientshare_photo_disabled[$i];
			$update_clientshare_photo_data['clientshare_id'] = $clientshare_id;
			$update_clientshare_photo_data['create_date'] = 'null';
			$update_clientshare_photo_data['create_by'] = '';
			$update_clientshare_photo_data['last_update_by'] = '';
			$G_DB_CONNECT->query_insert(TB_CLIENTSHARE_PHOTO, $update_clientshare_photo_data);
			////////////////////////////////////////////////
...
echo $G_DB_CONNECT->query_update_lang_content(TB_CLIENTSHARE, $update_data_lang, "  clientshare_id='".$update_data['id']."'   ");
*/
...
<?php 
   	...
   	printLangInputField(TITLE_SEO_TITLE,"seo_title",TB_CLIENTSHARE,$edit_data['id'],"seo_title",false,"input_seo_title","",false,'class="seo_row"');
...
printLangTextAreaNormal(TITLE_SEO_DESC,"seo_desc",TB_CLIENTSHARE,$edit_data['id'],false,300,450,'','class="seo_row"');
?>
...
<tr>
    ...
    <?php
		if(ACTION == '1'){
			$edit_data['sort_order'] = getNextSortOrder(TB_CLIENTSHARE,"");
		}
	?>
...
<tr style="display:none">
    ...
    <?php
		if(ACTION == '1'){
			//$edit_data['code'] = nextRecordcode(TB_CLIENTSHARE,"","",4);
		}
	?>
...
<?php 
   	...
   	printLangInputField(TITLE_TITLE,"name",TB_CLIENTSHARE,$edit_data['id'],"name",true);
	//printLangTextAreaNormal('Brief',"detail2",TB_CLIENTSHARE,$edit_data['id'],false,150,500,"");
	printLangTextArea(TITLE_DESC,"detail",TB_CLIENTSHARE,$edit_data['id'],true,500,600);
...
<input name="website" id="website"  class="input_middle"   value="<?php echo $edit_data['website']; ?>"/>
 ...
 <?php //echo printLangTextArea(TITLE_CONTENT_IF_NO_LINK,"detail",TB_CLIENTSHARE,$edit_data['id'],false,800); ?>  

   
<?php
// printLangTextAreaNormal(TITLE_TITLE,"name",TB_CLIENTSHARE,$edit_data['id'],false,150,500,"");
?>
...
<?php
 //printLangTextAreaNormal(TITLE_ADDRESS,"address",TB_CLIENTSHARE,$edit_data['id'],false,150,500,"");
?>
...
<table border="0" cellspacing="2" cellpadding="0"  id="table_text2">
  ...
  <tr>
     ...
     <br>
     <?php echo NOTICE_BEST_VIEW_PART_1; ?><?php echo TITLE_WIDTH; ?> : 570,<?php echo TITLE_HEIGHT; ?> : 320 <?php echo NOTICE_BEST_VIEW_PART_2; ?> 
     <br>
...
<?php
	$this_dynamic_area_id = "dynamic_clientshare_photo";
?>
...
<input name="clientshare_photo_img" id="clientshare_photo_img"  value="" label="<?php echo TITLE_IMG;?>" type="file" class="input_file file_upload"/>
<input name="clientshare_photo_img_old_path"  id="clientshare_photo_img_old_path" value="" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('clientshare_photo_language_id', $sql,'name','0','','','','','');
?>
    
    </td>
    
<td  style="display:none"><input name="clientshare_photo_sort_order" id="clientshare_photo_sort_order"  value="" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
  ...
  $sql = "select status_disable.*, status_disable_desc.name as name from ".TB_STATUS_DISABLE." as status_disable , ".TB_STATUS_DISABLE_DESC." as status_disable_desc ";
	...
	printCustomMenu('clientshare_photo_disabled', $sql,'name','0','','','','','');
?>
    ...
    <td style="width:250px;"><a href="#"  style="display:none"  class="button btn_remove_clientshare_photo"   ><span><?php echo BTN_REMOVE;?></span></a></td>
  </tr>
...
<?php
	...
	$i = 0;
  	$sql = "select * from ".TB_CLIENTSHARE_PHOTO." as clientshare_photo  ";
	$sql .= " where clientshare_id='".ID."' ";
	...
	if($G_DB_CONNECT->affected_rows > 0){
			while($data = $G_DB_CONNECT->fetch_array($rows)){
				...
				$clientshare_photo_img = "../".$data['img'];
				$arr_file_info = getImageInfo($clientshare_photo_img,"thumb",$photo_width,$photo_height);
				$clientshare_photo_img_thumb = $arr_file_info["src"];
...
<?php
	...
	$photo_height = 70;
				...
				$clientshare_photo_img = "../".$data['thumb'];
				$arr_file_info = getImageInfo($clientshare_photo_img,"thumb",$photo_width,$photo_height);
				$clientshare_photo_img_thumb = $arr_file_info["src"];
...
<a href="<?php echo $clientshare_photo_img; ?>" target="_blank"><img src="<?php echo $clientshare_photo_img_thumb?>"  style="margin-bottom:10px;" align="middle" width="<?php echo $arr_file_info['width'] ?>" height="<?php echo $arr_file_info['height'] ?>"/></a>

</div>
</div>
<br>
<input name="clientshare_photo_img[]" id="clientshare_photo_img[]"  value="" label="<?php echo TITLE_PHOTO;?>" type="file" class="input_file img_upload"/>
<input name="clientshare_photo_img_old_path[]"  id="clientshare_photo_img_old_path[]" value="<?php echo $data['img']; ?>" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('clientshare_photo_language_id[]', $sql,'name','0',$data['language_id'],'','','','');
?>
   ...
   <td  style="display:none"><input name="clientshare_photo_sort_order[]" id="clientshare_photo_sort_order[]"  value="<?php echo $data['sort_order']; ?>" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
	...
	printCustomMenu('clientshare_photo_disabled[]', $sql,'name','0',$data['disabled'],'','','','');
?>
...
?>
  ...
  <tr id="dynamic_clientshare_photo_footer" style="display:none">
   <td colspan="5">&nbsp;</td>
  </tr>
...
<div style="float:left;"><a  style="display:none" href="#" id="btn_add_clientshare_photo" class="button btn_add_clientshare_photo"  ><span><?php echo BTN_ADD_PHOTO;?></span></a></div>

...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and clientshare.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_CLIENTSHARE,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_CLIENTSHARE,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_CLIENTSHARE,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
//	$return_data['table_list_content'] .= '<th width="100"><a href="#" class="orderby_title" orderby="display_date"><span>'.TITLE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select clientshare.*,clientshare_desc.name as name from ".TB_CLIENTSHARE." as clientshare, ".TB_CLIENTSHARE_DESC." as clientshare_desc ";
	$sql .= " where ";
	$sql .= " clientshare.id = clientshare_desc.clientshare_id ";
	$sql .= " and clientshare.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and clientshare_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by clientshare.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_CLIENTSHARE_PHOTO," clientshare_id='".$data['id']."' ","thumb",200,100);

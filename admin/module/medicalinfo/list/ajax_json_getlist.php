...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['update_sort_order'] > 0 && $_REQUEST['update_sort_order_data_id'] > 0){
		...
		$sql = " update ".TB_MEDICALINFO;
		$sql .= " set ";
		$sql .= " sort_order = '".$_REQUEST['update_sort_order']."' ";
		$sql .= " where ";
		$sql .= " id = '".$_REQUEST['update_sort_order_data_id']."' ";
		$G_DB_CONNECT->query($sql);
		renewSortOrder(TB_MEDICALINFO," medicalinfo_category_id='".$_REQUEST['update_sort_order_category_id']."'");
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and medicalinfo.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_MEDICALINFO,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_MEDICALINFO,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_MEDICALINFO,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
$return_data['table_list_content'] .= ' <th width="100">&nbsp;</th>';
	...
	//$return_data['table_list_content'] .= ' <th><a href="#" class="orderby_title" orderby="tel"><span>'.TITLE_TEL.'</span></a></th>';
			 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="medicalinfo_category_id"><span>'.TITLE_CATEGORY.'</span></a></th>';
 ...
 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="sort_order"><span>'.TITLE_SORT_ORDER.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select medicalinfo.*,medicalinfo_desc.name as name,medicalinfo_desc.name2,medicalinfo_desc.name3 from ".TB_MEDICALINFO." as medicalinfo, ".TB_MEDICALINFO_DESC." as medicalinfo_desc ";
	$sql .= " where ";
	$sql .= " medicalinfo.id = medicalinfo_desc.medicalinfo_id ";
	$sql .= " and medicalinfo.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and medicalinfo_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by medicalinfo.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_MEDICALINFO_PHOTO," medicalinfo_id='".$data['id']."' ","thumb",100,500);
...
//	 $return_data['table_list_content'] .= '<td>'.$data['display_date'].'</td>';
    ...
    $return_data['table_list_content'] .= '<td>';
	/*
	$return_data['table_list_content'] .= nl2br($data['name2']);
		$return_data['table_list_content'] .= '<br>';
		$return_data['table_list_content'] .= '<span style="font-size:24px;line-height:120%;">'.nl2br($data['name3']).'</span>';
		$return_data['table_list_content'] .= '<br>';
		*/
	...
	//$return_data['table_list_content'] .= '<td>'.$data['tel'].'</td>';
		 $return_data['table_list_content'] .= '<td>'.getLangName(TB_MEDICALINFO_CATEGORY,"name",$data['medicalinfo_category_id'],ADMIN_LANG_ID).'</td>';
		 
	$return_data['table_list_content'] .= '<td>';
	

	$return_data['table_list_content'] .= printSortOrderInput($data['id'],$data['sort_order'],$data['medicalinfo_category_id']);

...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and trading_support.disabled = '".$_REQUEST['search_disabled']."' ";
		}


		if($_REQUEST['search_trading_support_category_id'] != '-1'){
			$search_condition .= " and trading_support.trading_support_category_id = '".$_REQUEST['search_trading_support_category_id']."' ";
		}

/*
				if($_REQUEST['search_product_id'] != '-1'){
			
			$search_condition .= " and trading_support.id in (select trading_support_id from ".TB_TRADING_SUPPORT_TO_PRODUCT." where product_id='".$_REQUEST['search_product_id']."' )";
		}
		if($_REQUEST['search_product_type_id'] != '-1'){
			
			$search_condition .= " and trading_support.id in (select trading_support_id from ".TB_TRADING_SUPPORT_TO_PRODUCT_TYPE." where product_type_id='".$_REQUEST['search_product_type_id']."' )";
		}
		
				if($_REQUEST['search_product_category_id'] != '-1'){
			
			$search_condition .= " and trading_support.id in (select trading_support_id from ".TB_TRADING_SUPPORT_TO_PRODUCT_CATEGORY." where product_category_id='".$_REQUEST['search_product_category_id']."' )";
		}
		
				if($_REQUEST['search_product_category2_id'] != '-1'){
			
			$search_condition .= " and trading_support.id in (select trading_support_id from ".TB_TRADING_SUPPORT_TO_PRODUCT_CATEGORY2." where product_category2_id='".$_REQUEST['search_product_category2_id']."' )";
		}
				if($_REQUEST['search_product_category3_id'] != '-1'){
			
			$search_condition .= " and trading_support.id in (select trading_support_id from ".TB_TRADING_SUPPORT_TO_PRODUCT_CATEGORY3." where product_category3_id='".$_REQUEST['search_product_category3_id']."' )";
		}
		*/
	}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_TRADING_SUPPORT,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_TRADING_SUPPORT,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_TRADING_SUPPORT,$record_id,true,'');
		}
		generateHTACCESS();
	}
    ...
    $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="name"><span>'.TITLE_TITLE.'</span></a></th>';
	 ...
	 //   $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="trading_support_category_id"><span>类别</span></a></th>';
  ...
  $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="disabled"><span>'.TITLE_DISABLED.'</span></a></th>';
   ...
   //$return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="create_date"><span>'.TITLE_CREATE_DATE_BRIEF.'</span></a> <a href="#" class="orderby_title" orderby="last_update_date"><span>'.TITLE_LAST_UPDATE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select trading_support.*,trading_support_desc.name as name from ".TB_TRADING_SUPPORT." as trading_support, ".TB_TRADING_SUPPORT_DESC." as trading_support_desc ";
	$sql .= " where ";
	$sql .= " trading_support.id = trading_support_desc.trading_support_id ";
	$sql .= " and trading_support.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and trading_support_desc.language_id = '".ADMIN_LANG_ID."'";
	
	$sql .= " and trading_support.trading_support_category_id = 1 ";
	
	$sql .= $search_condition;
	//$sql .= " group by trading_support.id  ";
...
/*
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_TRADING_SUPPORT_PHOTO," trading_support_id='".$data['id']."' ","thumb",100,500);
	...
	$return_data['table_list_content'] .=  getLangName(TB_TRADING_SUPPORT_CATEGORY,"name",$data['trading_support_category_id'],CURRENT_LANG);
		$return_data['table_list_content'] .= '</td>';
	*/
	...
	if($G_DB_CONNECT->affected_rows > 0){
		...
		while($data2a = $G_DB_CONNECT->fetch_array($rows2a)){
			...
			$sql .= " where ";
			$sql .= " trading_support_id = '".$data['id']."' ";

...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and video.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_VIDEO,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_VIDEO,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_VIDEO,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
$return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="sort_order"><span>'.TITLE_SORT_ORDER.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select video.*,video_desc.name as name from ".TB_VIDEO." as video, ".TB_VIDEO_DESC." as video_desc ";
	$sql .= " where ";
	$sql .= " video.id = video_desc.video_id ";
	$sql .= " and video.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and video_desc.language_id = '".ADMIN_LANG_ID."'";
...
//////////////////////////////////////////////////////////
	...
	*/
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_VIDEO_PHOTO," video_id='".$data['id']."' ","thumb",100,500);
...
//	}
	...
	if($data['youtube_id'] != ''){
		$return_data['table_list_content'] .= $data['youtube_id'];
	}

...
//	$_REQUEST['id'] = 1;
	...
	$sql = "select * from ".TB_VIDEO." as video ";
	$sql .= " where ";
	$sql .= " id='".ID."' ";
...
// upload file
/////////////////////////////////////////////
$arr_video_photo_language_id = getRequestVar("video_photo_language_id",'');
$arr_video_photo_sort_order = getRequestVar("video_photo_sort_order",'');
$arr_video_photo_caption = getRequestVar("video_photo_caption",'');
$arr_video_photo_disabled = getRequestVar("video_photo_disabled",'');
$arr_video_photo_img_old_path = getRequestVar("video_photo_img_old_path",'');
$arr_video_photo_thumb_old_path = getRequestVar("video_photo_thumb_old_path",'');
if($arr_video_photo_disabled != '' && ID > 0){

	$video_id = ID;
	//$video_code = getDataName(TB_VIDEO,"code",$video_id);
	$sql = "delete from ".TB_VIDEO_PHOTO." where video_id='".$video_id."'";
	$G_DB_CONNECT->query($sql);

	for ($i=0; $i<count($arr_video_photo_disabled); $i++) {




		$video_photo_img_old_path = $arr_video_photo_img_old_path[$i];
		$video_photo_img = $video_photo_img_old_path;

		$dir = "upload/images/video/";
		$file_name = "video_photo_img";
		////////////////////////////////////////////////
		$tempFile = $_FILES[$file_name]['tmp_name'][$i];
		if($tempFile != ''){
			$targetPath = CURRENT_DOCUMENT_ROOT . $dir ;
			$ext = get_extension($_FILES[$file_name]['name'][$i]);
			//$new_file_name = $video_code."_".date("YmdHis")."_".getLastID(TB_VIDEO_PHOTO).$ext;
			$new_file_name = date("YmdHis")."_".getLastID(TB_VIDEO_PHOTO).$ext;
  			$targetFile =  str_replace('//','/',$targetPath) .$new_file_name ;
			move_uploaded_file($tempFile,$targetFile);
			$targetFile =$dir.$new_file_name;
			////////////////////////////////////////////////

			$video_photo_img  = $targetFile;
			
			////////////////////////////////////////////////
			smart_resize_image( "../".$targetFile,"thumb", 5000, 1080, true);
			smart_resize_image( "../".$targetFile,"img",5000, 1080, true);
		...
		if($video_photo_img != '' ){
			////////////////////////////////////////////////
			$update_video_photo_data = array();
			$update_video_photo_data['language_id'] = $arr_video_photo_language_id[$i];
			$update_video_photo_data['img'] = $video_photo_img;
			$update_video_photo_data['thumb'] = $video_photo_thumb;
			$update_video_photo_data['caption'] = $arr_video_photo_caption[$i];
			$update_video_photo_data['sort_order'] = $arr_video_photo_sort_order[$i];
			$update_video_photo_data['disabled'] = $arr_video_photo_disabled[$i];
			$update_video_photo_data['video_id'] = $video_id;
			$update_video_photo_data['create_date'] = 'null';
			$update_video_photo_data['create_by'] = '';
			$update_video_photo_data['last_update_by'] = '';
			$G_DB_CONNECT->query_insert(TB_VIDEO_PHOTO, $update_video_photo_data);
			////////////////////////////////////////////////
...
echo $G_DB_CONNECT->query_update_lang_content(TB_VIDEO, $update_data_lang, "  video_id='".$update_data['id']."'   ");
*/
...
<?php 
   	...
   	printLangInputField(TITLE_SEO_TITLE,"seo_title",TB_VIDEO,$edit_data['id'],"seo_title",false,"input_seo_title","",false,'class="seo_row"');
...
printLangTextAreaNormal(TITLE_SEO_DESC,"seo_desc",TB_VIDEO,$edit_data['id'],false,300,450,'','class="seo_row"');
?>
...
<tr>
    ...
    <?php
		if(ACTION == '1'){
			$edit_data['sort_order'] = getNextSortOrder(TB_VIDEO,"");
		}
	?>
...
<tr style="display:none">
    ...
    <?php
		if(ACTION == '1'){
			//$edit_data['code'] = nextRecordcode(TB_VIDEO,"","",4);
		}
	?>
...
<?php 
  ...
  // 	printLangInputField(TITLE_TITLE,"name",TB_VIDEO,$edit_data['id'],"name",true);
	//printLangTextAreaNormal('Brief',"detail2",TB_VIDEO,$edit_data['id'],false,150,500,"");
	//printLangTextArea(TITLE_DESC,"detail",TB_VIDEO,$edit_data['id'],true,1300,850);
...
<input  name="youtube_id" id="youtube_id" class="input_middle"  required="yes" label="影片連結"  value="<?php echo $edit_data['youtube_id']; ?>"/>
 ...
 <?php //echo printLangTextArea(TITLE_CONTENT_IF_NO_LINK,"detail",TB_VIDEO,$edit_data['id'],false,800); ?>  

   
<?php
// printLangTextAreaNormal(TITLE_TITLE,"name",TB_VIDEO,$edit_data['id'],false,150,500,"");
?>
...
<?php
 //printLangTextAreaNormal(TITLE_ADDRESS,"address",TB_VIDEO,$edit_data['id'],false,150,500,"");
?>
...
<table border="0" cellspacing="2" cellpadding="0"  id="table_text2">
  ...
  <tr>
     ...
     <br>
     <?php echo NOTICE_BEST_VIEW_PART_1; ?><?php echo TITLE_WIDTH; ?> : 1920,<?php echo TITLE_HEIGHT; ?> : 1080 <?php echo NOTICE_BEST_VIEW_PART_2; ?> 
     <br>
...
<?php
	$this_dynamic_area_id = "dynamic_video_photo";
?>
...
<input name="video_photo_img" id="video_photo_img"  value="" label="<?php echo TITLE_IMG;?>" type="file" class="input_file file_upload"/>
<input name="video_photo_img_old_path"  id="video_photo_img_old_path" value="" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('video_photo_language_id', $sql,'name','0','','','','','');
?>
    
    </td>
    
<td style="display:none"><input name="video_photo_sort_order" id="video_photo_sort_order"  value="" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
  ...
  $sql = "select status_disable.*, status_disable_desc.name as name from ".TB_STATUS_DISABLE." as status_disable , ".TB_STATUS_DISABLE_DESC." as status_disable_desc ";
	...
	printCustomMenu('video_photo_disabled', $sql,'name','0','','','','','');
?>
    ...
    <td style="width:250px;"><a href="#"  style="display:none" class="button btn_remove_video_photo"   ><span><?php echo BTN_REMOVE;?></span></a></td>
  </tr>
...
<?php
	...
	$i = 0;
  	$sql = "select * from ".TB_VIDEO_PHOTO." as video_photo  ";
	...
	if($G_DB_CONNECT->affected_rows > 0){
			while($data = $G_DB_CONNECT->fetch_array($rows)){
				...
				$video_photo_img = "../".$data['img'];
				$arr_file_info = getImageInfo($video_photo_img,"thumb",$photo_width,$photo_height);
				$video_photo_img_thumb = $arr_file_info["src"];
...
<a href="<?php echo $video_photo_img; ?>" target="_blank"><img src="<?php echo $video_photo_img_thumb?>"  style="margin-bottom:10px;" align="middle" width="<?php echo $arr_file_info['width'] ?>" height="<?php echo $arr_file_info['height'] ?>"/></a>

</div>
</div>
<br>
<input name="video_photo_img[]" id="video_photo_img[]"  value="" label="<?php echo TITLE_PHOTO;?>" type="file" class="input_file img_upload"/>
<input name="video_photo_img_old_path[]"  id="video_photo_img_old_path[]" value="<?php echo $data['img']; ?>" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('video_photo_language_id[]', $sql,'name','0',$data['language_id'],'','','','');
?>
   ...
   <td style="display:none"><input name="video_photo_sort_order[]" id="video_photo_sort_order[]"  value="<?php echo $data['sort_order']; ?>" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
	...
	printCustomMenu('video_photo_disabled[]', $sql,'name','0',$data['disabled'],'','','','');
?>
...
?>
  ...
  <tr id="dynamic_video_photo_footer" style="display:none">
   <td colspan="5">&nbsp;</td>
  </tr>
...
<div style="float:left;"><a href="#" style="display:none" id="btn_add_video_photo" class="button btn_add_video_photo"  ><span><?php echo BTN_ADD_PHOTO;?></span></a></div>

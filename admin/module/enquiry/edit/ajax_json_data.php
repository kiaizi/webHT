...
//////////////////////////////////////////////////////////
	if(ACTION == '1'){
		...
		$sql .= " where ";
		$sql .= "  enquiry.username='$username'  ";
		$sql .= " and enquiry.disabled<>'".DISABLED_DELETE."' ";
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_ENQUIRY, $update_data); 
		}
	...
	if(ACTION == '2'){
		$sql = "select enquiry.* from ".TB_ENQUIRY." as enquiry ";
		$sql .= " where ";
		$sql .= "  enquiry.username='$username'  and enquiry.id <> '".$update_data['id']."' ";
		$sql .= " and enquiry.disabled<>'".DISABLED_DELETE."' ";
		$sql .= " group by enquiry.id  ";
		$rows = $G_DB_CONNECT->query($sql);
		if($G_DB_CONNECT->affected_rows > 0){
			...
			$sql = "select enquiry.* from ".TB_ENQUIRY." as enquiry ";
			$sql .= " where ";
			$sql .= " enquiry.code='$code'  and enquiry.id <> '".$update_data['id']."' ";
			$sql .= " and enquiry.disabled<>'".DISABLED_DELETE."' ";
			$sql .= " group by enquiry.id  ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_ENQUIRY, $update_data, "id='".$update_data['id']."'"); 
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_ENQUIRY,$update_data['id'],true,'');

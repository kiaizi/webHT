...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select faq_category.* from ".TB_FAQ_CATEGORY." as faq_category, ".TB_FAQ_CATEGORY_DESC." as faq_category_desc ";
			$sql .= " where ";
			$sql .= " faq_category_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and faq_category_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and faq_category_desc.faq_category_id=faq_category.id ";
			$sql .= " and faq_category.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " faq_category.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_FAQ_CATEGORY, $update_data); 
			$update_data_lang['faq_category_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_FAQ_CATEGORY, $update_data_lang);
...
/*
			...
			$sql .= " and faq_category_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and faq_category_desc.faq_category_id=faq_category.id ";
			$sql .= " and faq_category.id<>'".$update_data['id']."'  ";
			$sql .= " and faq_category.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_FAQ_CATEGORY, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['faq_category_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_FAQ_CATEGORY, $update_data_lang, "  faq_category_id='".$update_data['id']."'   ");
		}
	...
	if($update_data['seo_url'] == ''){
		$G_DB_CONNECT->query("update ".TB_FAQ_CATEGORY." set seo_url='".$return_data['id']."'  where id='".$return_data['id']."' ");
	}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_FAQ_CATEGORY,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_FAQ_CATEGORY," id>0 ");

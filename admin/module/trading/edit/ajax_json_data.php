...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select trading.* from ".TB_TRADING." as trading, ".TB_TRADING_DESC." as trading_desc ";
			$sql .= " where ";
			$sql .= " trading_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and trading_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and trading_desc.trading_id=trading.id ";
			$sql .= " and trading.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " trading.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_TRADING, $update_data); 
			$update_data_lang['trading_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_TRADING, $update_data_lang);
...
/*
			...
			$sql .= " and trading_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and trading_desc.trading_id=trading.id ";
			$sql .= " and trading.id<>'".$update_data['id']."'  ";
			$sql .= " and trading.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_TRADING, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['trading_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_TRADING, $update_data_lang, "  trading_id='".$update_data['id']."'   ");
		}
	...
	if($update_data['seo_url'] == ''){
		$G_DB_CONNECT->query("update ".TB_TRADING." set seo_url='".$return_data['id']."'  where id='".$return_data['id']."' ");
	}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_TRADING,$update_data['id'],true,'');

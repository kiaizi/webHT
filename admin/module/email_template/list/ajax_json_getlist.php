...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and email_template.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_EMAIL_TEMPLATE,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_EMAIL_TEMPLATE,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_EMAIL_TEMPLATE,$record_id,true,'');
		}
		generateHTACCESS();
	}
  ...
  //	$return_data['table_list_content'] .= '<th class="th_check_all"><input name="check_all"  id="check_all" type="checkbox" value="" /></th>';
	...
	$return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="email_to_name"><span>Email To/From Name</span></a></th>';
	$return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="email_to"><span>Email To/From</span></a></th>';
   ...
   // $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="disabled"><span>'.TITLE_DISABLED.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select email_template.*,email_template_desc.name as name,email_template_desc.name2 from ".TB_EMAIL_TEMPLATE." as email_template, ".TB_EMAIL_TEMPLATE_DESC." as email_template_desc ";
	$sql .= " where ";
	$sql .= " email_template.id = email_template_desc.email_template_id ";
	$sql .= " and email_template.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and email_template_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by email_template.id  ";
...
/*
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_EMAIL_TEMPLATE_PHOTO," email_template_id='".$data['id']."' ","thumb",100,500);
    ...
    $return_data['table_list_content'] .= '<td style="padding-left:25px;">';
	...
	$return_data['table_list_content'] .= '</td>';
	  $return_data['table_list_content'] .= '<td>';
		...
		$return_data['table_list_content'] .= nl2br($data['email_to_name']);
	...
	$return_data['table_list_content'] .= '</td>';
	  $return_data['table_list_content'] .= '<td>';
		...
		$return_data['table_list_content'] .= nl2br($data['email_to']);

...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and deposit.disabled = '".$_REQUEST['search_disabled']."' ";
		}


		if($_REQUEST['search_deposit_category_id'] != '-1'){
			$search_condition .= " and deposit.deposit_category_id = '".$_REQUEST['search_deposit_category_id']."' ";
		}

/*
				if($_REQUEST['search_product_id'] != '-1'){
			
			$search_condition .= " and deposit.id in (select deposit_id from ".TB_DEPOSIT_TO_PRODUCT." where product_id='".$_REQUEST['search_product_id']."' )";
		}
		if($_REQUEST['search_product_type_id'] != '-1'){
			
			$search_condition .= " and deposit.id in (select deposit_id from ".TB_DEPOSIT_TO_PRODUCT_TYPE." where product_type_id='".$_REQUEST['search_product_type_id']."' )";
		}
		
				if($_REQUEST['search_product_category_id'] != '-1'){
			
			$search_condition .= " and deposit.id in (select deposit_id from ".TB_DEPOSIT_TO_PRODUCT_CATEGORY." where product_category_id='".$_REQUEST['search_product_category_id']."' )";
		}
		
				if($_REQUEST['search_product_category2_id'] != '-1'){
			
			$search_condition .= " and deposit.id in (select deposit_id from ".TB_DEPOSIT_TO_PRODUCT_CATEGORY2." where product_category2_id='".$_REQUEST['search_product_category2_id']."' )";
		}
				if($_REQUEST['search_product_category3_id'] != '-1'){
			
			$search_condition .= " and deposit.id in (select deposit_id from ".TB_DEPOSIT_TO_PRODUCT_CATEGORY3." where product_category3_id='".$_REQUEST['search_product_category3_id']."' )";
		}
		*/
	}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_DEPOSIT,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_DEPOSIT,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_DEPOSIT,$record_id,true,'');
		}
		generateHTACCESS();
	}
    ...
    $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="name"><span>'.TITLE_TITLE.'</span></a></th>';
	 ...
	 //   $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="deposit_category_id"><span>类别</span></a></th>';
  ...
  $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="disabled"><span>'.TITLE_DISABLED.'</span></a></th>';
   ...
   //$return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="create_date"><span>'.TITLE_CREATE_DATE_BRIEF.'</span></a> <a href="#" class="orderby_title" orderby="last_update_date"><span>'.TITLE_LAST_UPDATE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select deposit.*,deposit_desc.name as name from ".TB_DEPOSIT." as deposit, ".TB_DEPOSIT_DESC." as deposit_desc ";
	$sql .= " where ";
	$sql .= " deposit.id = deposit_desc.deposit_id ";
	$sql .= " and deposit.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and deposit_desc.language_id = '".ADMIN_LANG_ID."'";
	
	$sql .= " and deposit.deposit_category_id = 1 ";
	
	$sql .= $search_condition;
	//$sql .= " group by deposit.id  ";
...
/*
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_DEPOSIT_PHOTO," deposit_id='".$data['id']."' ","thumb",100,500);
	...
	$return_data['table_list_content'] .=  getLangName(TB_DEPOSIT_CATEGORY,"name",$data['deposit_category_id'],CURRENT_LANG);
		$return_data['table_list_content'] .= '</td>';
	*/

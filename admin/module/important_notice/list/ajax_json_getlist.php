...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['update_sort_order'] > 0 && $_REQUEST['update_sort_order_data_id'] > 0){
		...
		$sql = " update ".TB_IMPORTANT_NOTICE;
		$sql .= " set ";
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and important_notice.disabled = '".$_REQUEST['search_disabled']."' ";
		}


		if($_REQUEST['search_important_notice_category_id'] != '-1'){
			$search_condition .= " and important_notice.important_notice_category_id = '".$_REQUEST['search_important_notice_category_id']."' ";
		}

/*
				if($_REQUEST['search_product_id'] != '-1'){
			
			$search_condition .= " and important_notice.id in (select important_notice_id from ".TB_IMPORTANT_NOTICE_TO_PRODUCT." where product_id='".$_REQUEST['search_product_id']."' )";
		}
		if($_REQUEST['search_product_type_id'] != '-1'){
			
			$search_condition .= " and important_notice.id in (select important_notice_id from ".TB_IMPORTANT_NOTICE_TO_PRODUCT_TYPE." where product_type_id='".$_REQUEST['search_product_type_id']."' )";
		}
		
				if($_REQUEST['search_product_category_id'] != '-1'){
			
			$search_condition .= " and important_notice.id in (select important_notice_id from ".TB_IMPORTANT_NOTICE_TO_PRODUCT_CATEGORY." where product_category_id='".$_REQUEST['search_product_category_id']."' )";
		}
		
				if($_REQUEST['search_product_category2_id'] != '-1'){
			
			$search_condition .= " and important_notice.id in (select important_notice_id from ".TB_IMPORTANT_NOTICE_TO_PRODUCT_CATEGORY2." where product_category2_id='".$_REQUEST['search_product_category2_id']."' )";
		}
				if($_REQUEST['search_product_category3_id'] != '-1'){
			
			$search_condition .= " and important_notice.id in (select important_notice_id from ".TB_IMPORTANT_NOTICE_TO_PRODUCT_CATEGORY3." where product_category3_id='".$_REQUEST['search_product_category3_id']."' )";
		}
		*/
	}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_IMPORTANT_NOTICE,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_IMPORTANT_NOTICE,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_IMPORTANT_NOTICE,$record_id,true,'');
		}
		generateHTACCESS();
	}
    ...
    $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="name"><span>'.TITLE_TITLE.'</span></a></th>';
	    ...
	    $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="important_notice_category_id"><span>类别</span></a></th>';
  ...
  $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="disabled"><span>'.TITLE_DISABLED.'</span></a></th>';
   ...
   //$return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="create_date"><span>'.TITLE_CREATE_DATE_BRIEF.'</span></a> <a href="#" class="orderby_title" orderby="last_update_date"><span>'.TITLE_LAST_UPDATE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select important_notice.*,important_notice_desc.name as name from ".TB_IMPORTANT_NOTICE." as important_notice, ".TB_IMPORTANT_NOTICE_DESC." as important_notice_desc ";
	$sql .= " where ";
	$sql .= " important_notice.id = important_notice_desc.important_notice_id ";
	$sql .= " and important_notice.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and important_notice_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by important_notice.id  ";
...
/*
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_IMPORTANT_NOTICE_PHOTO," important_notice_id='".$data['id']."' ","thumb",100,500);
	...
	$return_data['table_list_content'] .=  getLangName(TB_IMPORTANT_NOTICE_CATEGORY,"name",$data['important_notice_category_id'],CURRENT_LANG);
		$return_data['table_list_content'] .= '</td>';
	...
	*/
	...
	if($G_DB_CONNECT->affected_rows > 0){
		...
		while($data2a = $G_DB_CONNECT->fetch_array($rows2a)){
			...
			$sql .= " where ";
			$sql .= " important_notice_id = '".$data['id']."' ";

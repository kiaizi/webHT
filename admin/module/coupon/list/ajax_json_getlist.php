...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and coupon.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_COUPON,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_COUPON,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_COUPON,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
//	$return_data['table_list_content'] .= '<th width="100"><a href="#" class="orderby_title" orderby="display_date"><span>'.TITLE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select coupon.*,coupon_desc.name as name from ".TB_COUPON." as coupon, ".TB_COUPON_DESC." as coupon_desc ";
	$sql .= " where ";
	$sql .= " coupon.id = coupon_desc.coupon_id ";
	$sql .= " and coupon.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and coupon_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by coupon.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_COUPON_PHOTO," coupon_id='".$data['id']."' ","thumb",200,100);

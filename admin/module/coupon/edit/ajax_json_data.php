...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select coupon.* from ".TB_COUPON." as coupon, ".TB_COUPON_DESC." as coupon_desc ";
			$sql .= " where ";
			$sql .= " coupon_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and coupon_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and coupon_desc.coupon_id=coupon.id ";
			$sql .= " and coupon.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " coupon.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_COUPON, $update_data); 
			$update_data_lang['coupon_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_COUPON, $update_data_lang);
...
/*
			...
			$sql .= " and coupon_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and coupon_desc.coupon_id=coupon.id ";
			$sql .= " and coupon.id<>'".$update_data['id']."'  ";
			$sql .= " and coupon.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_COUPON, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['coupon_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_COUPON, $update_data_lang, "  coupon_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_COUPON,$update_data['id'],true,'');

<?php 
	...
	$sql = "select * from ".TB_COUPON." as coupon ";
	$sql .= " where ";
	$sql .= " id='".ID."' ";
...
// upload file
/////////////////////////////////////////////
$arr_coupon_photo_language_id = getRequestVar("coupon_photo_language_id",'');
$arr_coupon_photo_sort_order = getRequestVar("coupon_photo_sort_order",'');
$arr_coupon_photo_caption = getRequestVar("coupon_photo_caption",'');
$arr_coupon_photo_disabled = getRequestVar("coupon_photo_disabled",'');
$arr_coupon_photo_img_old_path = getRequestVar("coupon_photo_img_old_path",'');
$arr_coupon_photo_thumb_old_path = getRequestVar("coupon_photo_thumb_old_path",'');
if($arr_coupon_photo_disabled != '' && ID > 0){

	$coupon_id = ID;
	//$coupon_code = getDataName(TB_COUPON,"code",$coupon_id);
	$sql = "delete from ".TB_COUPON_PHOTO." where coupon_id='".$coupon_id."'";
	$G_DB_CONNECT->query($sql);

	for ($i=0; $i<count($arr_coupon_photo_disabled); $i++) {




		$coupon_photo_img_old_path = $arr_coupon_photo_img_old_path[$i];
		$coupon_photo_img = $coupon_photo_img_old_path;

		$dir = "upload/images/coupon/";
		$file_name = "coupon_photo_img";
		////////////////////////////////////////////////
		$tempFile = $_FILES[$file_name]['tmp_name'][$i];
		if($tempFile != ''){
			$targetPath = CURRENT_DOCUMENT_ROOT . $dir ;
			$ext = get_extension($_FILES[$file_name]['name'][$i]);
			//$new_file_name = $coupon_code."_".date("YmdHis")."_".getLastID(TB_COUPON_PHOTO).$ext;
			$new_file_name = date("YmdHis")."_".getLastID(TB_COUPON_PHOTO).$ext;
  			$targetFile =  str_replace('//','/',$targetPath) .$new_file_name ;
			move_uploaded_file($tempFile,$targetFile);
			$targetFile =$dir.$new_file_name;
			////////////////////////////////////////////////

			$coupon_photo_img  = $targetFile;
		...
		if($coupon_photo_img != '' ){
			////////////////////////////////////////////////
			$update_coupon_photo_data = array();
			$update_coupon_photo_data['language_id'] = $arr_coupon_photo_language_id[$i];
			$update_coupon_photo_data['img'] = $coupon_photo_img;
			$update_coupon_photo_data['thumb'] = $coupon_photo_thumb;
			$update_coupon_photo_data['caption'] = $arr_coupon_photo_caption[$i];
			$update_coupon_photo_data['sort_order'] = $arr_coupon_photo_sort_order[$i];
			$update_coupon_photo_data['disabled'] = $arr_coupon_photo_disabled[$i];
			$update_coupon_photo_data['coupon_id'] = $coupon_id;
			$update_coupon_photo_data['create_date'] = 'null';
			$update_coupon_photo_data['create_by'] = '';
			$update_coupon_photo_data['last_update_by'] = '';
			$G_DB_CONNECT->query_insert(TB_COUPON_PHOTO, $update_coupon_photo_data);
			////////////////////////////////////////////////
...
echo $G_DB_CONNECT->query_update_lang_content(TB_COUPON, $update_data_lang, "  coupon_id='".$update_data['id']."'   ");
*/
...
<?php 
   	...
   	printLangInputField(TITLE_SEO_TITLE,"seo_title",TB_COUPON,$edit_data['id'],"seo_title",false,"input_seo_title","",false,'class="seo_row"');
...
printLangTextAreaNormal(TITLE_SEO_DESC,"seo_desc",TB_COUPON,$edit_data['id'],false,300,450,'','class="seo_row"');
?>
...
<tr>
    ...
    <?php
		if(ACTION == '1'){
			$edit_data['sort_order'] = getNextSortOrder(TB_COUPON,"");
		}
	?>
...
<tr style="display:none">
    ...
    <?php
		if(ACTION == '1'){
			//$edit_data['code'] = nextRecordcode(TB_COUPON,"","",4);
		}
	?>
...
<?php 
   	...
   	printLangInputField(TITLE_TITLE,"name",TB_COUPON,$edit_data['id'],"name",true);
	//printLangTextAreaNormal('Brief',"detail2",TB_COUPON,$edit_data['id'],false,150,500,"");
	printLangTextArea(TITLE_DESC,"detail",TB_COUPON,$edit_data['id'],true,800,520);
...
<input name="website" id="website"  class="input_middle"   value="<?php echo $edit_data['website']; ?>"/>
 ...
 <?php //echo printLangTextArea(TITLE_CONTENT_IF_NO_LINK,"detail",TB_COUPON,$edit_data['id'],false,800); ?>  

   
<?php
// printLangTextAreaNormal(TITLE_TITLE,"name",TB_COUPON,$edit_data['id'],false,150,500,"");
?>
...
<?php
 //printLangTextAreaNormal(TITLE_ADDRESS,"address",TB_COUPON,$edit_data['id'],false,150,500,"");
?>
...
<table border="0" cellspacing="2" cellpadding="0"  id="table_text2">
  ...
  <tr>
     ...
     <br>
     <?php echo NOTICE_BEST_VIEW_PART_1; ?><?php echo TITLE_WIDTH; ?> : 600,<?php echo TITLE_HEIGHT; ?> : 800 <?php echo NOTICE_BEST_VIEW_PART_2; ?> 
     <br>
...
<?php
	$this_dynamic_area_id = "dynamic_coupon_photo";
?>
...
<input name="coupon_photo_img" id="coupon_photo_img"  value="" label="<?php echo TITLE_IMG;?>" type="file" class="input_file file_upload"/>
<input name="coupon_photo_img_old_path"  id="coupon_photo_img_old_path" value="" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('coupon_photo_language_id', $sql,'name','0','','','','','');
?>
    
    </td>
    
<td  style="display:none"><input name="coupon_photo_sort_order" id="coupon_photo_sort_order"  value="" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
  ...
  $sql = "select status_disable.*, status_disable_desc.name as name from ".TB_STATUS_DISABLE." as status_disable , ".TB_STATUS_DISABLE_DESC." as status_disable_desc ";
	...
	printCustomMenu('coupon_photo_disabled', $sql,'name','0','','','','','');
?>
    ...
    <td style="width:250px;"><a href="#"  style="display:none"  class="button btn_remove_coupon_photo"   ><span><?php echo BTN_REMOVE;?></span></a></td>
  </tr>
...
<?php
	...
	$i = 0;
  	$sql = "select * from ".TB_COUPON_PHOTO." as coupon_photo  ";
	...
	if($G_DB_CONNECT->affected_rows > 0){
			while($data = $G_DB_CONNECT->fetch_array($rows)){
				...
				$coupon_photo_img = "../".$data['img'];
				$arr_file_info = getImageInfo($coupon_photo_img,"thumb",$photo_width,$photo_height);
				$coupon_photo_img_thumb = $arr_file_info["src"];
...
<a href="<?php echo $coupon_photo_img; ?>" target="_blank"><img src="<?php echo $coupon_photo_img_thumb?>"  style="margin-bottom:10px;" align="middle" width="<?php echo $arr_file_info['width'] ?>" height="<?php echo $arr_file_info['height'] ?>"/></a>

</div>
</div>
<br>
<input name="coupon_photo_img[]" id="coupon_photo_img[]"  value="" label="<?php echo TITLE_PHOTO;?>" type="file" class="input_file img_upload"/>
<input name="coupon_photo_img_old_path[]"  id="coupon_photo_img_old_path[]" value="<?php echo $data['img']; ?>" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('coupon_photo_language_id[]', $sql,'name','0',$data['language_id'],'','','','');
?>
   ...
   <td  style="display:none"><input name="coupon_photo_sort_order[]" id="coupon_photo_sort_order[]"  value="<?php echo $data['sort_order']; ?>" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
	...
	printCustomMenu('coupon_photo_disabled[]', $sql,'name','0',$data['disabled'],'','','','');
?>
...
?>
  ...
  <tr id="dynamic_coupon_photo_footer" style="display:none">
   <td colspan="5">&nbsp;</td>
  </tr>
...
<div style="float:left;"><a  style="display:none" href="#" id="btn_add_coupon_photo" class="button btn_add_coupon_photo"  ><span><?php echo BTN_ADD_PHOTO;?></span></a></div>

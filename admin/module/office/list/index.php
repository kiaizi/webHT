<?php 
	...
	include_once(DIR_COMMON."global_header.php");
		...
		if($_POST['upload_office_data_csv'] == '1'){
		$dir = "upload/files/office_price/";
		$file_name = "file_import_office_data_excel";
		////////////////////////////////////////////////
		$i=0;
		...
		if($tempFile != ''){
			...
			$new_file_name = "office_".date("YmdHis").$ext;
  			$targetFile =  str_replace('//','/',$targetPath) .$new_file_name ;
			move_uploaded_file($tempFile,$targetFile);
			$targetFile = "../".$dir.$new_file_name;
		
			/////////////////////////////////////////
			//$targetFile = "../files/office/sample.csv";
			//installProductDataExcel($targetFile);
			// $targetFile;
		installProductDataExcel($targetFile);
			/////////////////////////////////////////
		}
	...
	/*
	$sql = "select office.id,office.office_category_id from ".TB_OFFICE." as office , ".TB_OFFICE_DESC." as office_desc ";
		$sql .= " where ";
		$sql .= " office.id=office_desc.office_id ";
		$sql .= " and office_desc.language_id='".CURRENT_LANG."' ";
		$sql .= " and office.disabled='0' ";

		$sql .= " order by office.id asc   ";
		...
		if($G_DB_CONNECT->affected_rows > 0){
			while($data2 = $G_DB_CONNECT->fetch_array($rows2)){
				...
				$office_id = $data2['id'];
				$parent_office_category_id = $data2['office_category_id'];
				$office_category_id = 0;
				
		$sql = "select office_category.id from ".TB_OFFICE_CATEGORY." as office_category , ".TB_OFFICE_CATEGORY_DESC." as office_category_desc ";
		$sql .= " where ";
		$sql .= " office_category.id=office_category_desc.office_category_id ";
		$sql .= " and office_category_desc.language_id='".CURRENT_LANG."' ";
		$sql .= " and office_category.disabled='0' and parent_office_category_id='".$parent_office_category_id."' ";
		$sql .= " order by rand() limit 0,1 ";
		$rows2a = $G_DB_CONNECT->query($sql);

		if($G_DB_CONNECT->affected_rows > 0){
			while($data2a = $G_DB_CONNECT->fetch_array($rows2a)){
				$office_category_id = $data2a['id'];
				
				echo $sql = "update ".TB_OFFICE." set office_category_id='".$office_category_id."' where id='".$office_id."';";
				echo '<br>';
			}
		}
	...
	$sql = "select office.id,office.office_category_id from ".TB_OFFICE." as office , ".TB_OFFICE_DESC." as office_desc ";
		...
		if($G_DB_CONNECT->affected_rows > 0){
			while($data2 = $G_DB_CONNECT->fetch_array($rows2)){
				...
				$office_id = $data2['id'];
				$office_category_id = $data2['office_category_id'];
				$update_data = array();
				$update_data['office_id']=$office_id;
					$update_data['office_category_id']=$office_category_id;
				 $G_DB_CONNECT->query_insert(TB_OFFICE_TO_OFFICE_CATEGORY, $update_data); 
...
<input name="orderby" id="orderby" value="<?php echo getRequestVar('orderby','office_category_id asc,sort_order'); ?>" type="hidden" />
...
<input name="update_sort_order_category_id" id="update_sort_order_category_id" value="" type="hidden"/>



<input name="upload_office_data_csv" id="upload_office_data_csv" value="" type="hidden"/>

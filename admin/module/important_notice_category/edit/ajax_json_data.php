...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select important_notice_category.* from ".TB_IMPORTANT_NOTICE_CATEGORY." as important_notice_category, ".TB_IMPORTANT_NOTICE_CATEGORY_DESC." as important_notice_category_desc ";
			$sql .= " where ";
			$sql .= " important_notice_category_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and important_notice_category_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and important_notice_category_desc.important_notice_category_id=important_notice_category.id ";
			$sql .= " and important_notice_category.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " important_notice_category.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_IMPORTANT_NOTICE_CATEGORY, $update_data); 
			$update_data_lang['important_notice_category_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_IMPORTANT_NOTICE_CATEGORY, $update_data_lang);
...
/*
			...
			$sql .= " and important_notice_category_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and important_notice_category_desc.important_notice_category_id=important_notice_category.id ";
			$sql .= " and important_notice_category.id<>'".$update_data['id']."'  ";
			$sql .= " and important_notice_category.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_IMPORTANT_NOTICE_CATEGORY, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['important_notice_category_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_IMPORTANT_NOTICE_CATEGORY, $update_data_lang, "  important_notice_category_id='".$update_data['id']."'   ");
		}
	...
	if($update_data['seo_url'] == ''){
		$G_DB_CONNECT->query("update ".TB_IMPORTANT_NOTICE_CATEGORY." set seo_url='".$return_data['id']."'  where id='".$return_data['id']."' ");
	}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_IMPORTANT_NOTICE_CATEGORY,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_IMPORTANT_NOTICE_CATEGORY," id>0 ");

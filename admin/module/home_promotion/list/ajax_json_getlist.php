<?php 
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and home_promotion.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_HOME_PROMOTION,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_HOME_PROMOTION,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_HOME_PROMOTION,$record_id,true,'');
		}
		generateHTACCESS();
	}
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select home_promotion.*,home_promotion_desc.name as name from ".TB_HOME_PROMOTION." as home_promotion, ".TB_HOME_PROMOTION_DESC." as home_promotion_desc ";
	$sql .= " where ";
	$sql .= " home_promotion.id = home_promotion_desc.home_promotion_id ";
	$sql .= " and home_promotion.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and home_promotion_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by home_promotion.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_HOME_PROMOTION_PHOTO," home_promotion_id='".$data['id']."' ","thumb",200,1000);

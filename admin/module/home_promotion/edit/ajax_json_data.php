...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select home_promotion.* from ".TB_HOME_PROMOTION." as home_promotion, ".TB_HOME_PROMOTION_DESC." as home_promotion_desc ";
			$sql .= " where ";
			$sql .= " home_promotion_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and home_promotion_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and home_promotion_desc.home_promotion_id=home_promotion.id ";
			$sql .= " and home_promotion.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " home_promotion.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_HOME_PROMOTION, $update_data); 
			$update_data_lang['home_promotion_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_HOME_PROMOTION, $update_data_lang);
...
/*
			...
			$sql .= " and home_promotion_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and home_promotion_desc.home_promotion_id=home_promotion.id ";
			$sql .= " and home_promotion.id<>'".$update_data['id']."'  ";
			$sql .= " and home_promotion.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_HOME_PROMOTION, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['home_promotion_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_HOME_PROMOTION, $update_data_lang, "  home_promotion_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_HOME_PROMOTION,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_HOME_PROMOTION," id>0 ");

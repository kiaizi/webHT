...
if( $_SESSION['already_login']){
	...
	$trading_future_category_id =$update_data['trading_future_category_id'] = getRequestVar('trading_future_category_id','');

	//$trading_future_category_id = $update_data['trading_future_category_id'] = 1;
...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select trading_future.* from ".TB_TRADING_FUTURE." as trading_future, ".TB_TRADING_FUTURE_DESC." as trading_future_desc ";
			$sql .= " where ";
			$sql .= " trading_future_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and trading_future_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and trading_future_desc.trading_future_id=trading_future.id ";
			$sql .= " and trading_future.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " trading_future.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_TRADING_FUTURE, $update_data); 
			$update_data_lang['trading_future_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_TRADING_FUTURE, $update_data_lang);
...
/*
			...
			$sql .= " and trading_future_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and trading_future_desc.trading_future_id=trading_future.id ";
			$sql .= " and trading_future.id<>'".$update_data['id']."'  ";
			$sql .= " and trading_future.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_TRADING_FUTURE, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['trading_future_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_TRADING_FUTURE, $update_data_lang, "  trading_future_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_TRADING_FUTURE,$update_data['id'],true,'');
	...
	$G_DB_CONNECT->query("delete from ".TB_TRADING_FUTURE_TO_PRODUCT." where trading_future_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_id) > 0 && is_array($arr_product_id)  && $arr_product_id[0] != ''){
				for($i=0;$i<count($arr_product_id);$i++){
					...
					$update_data_product_to_product['trading_future_id'] = $update_data['id'];
					$update_data_product_to_product['product_id'] = $arr_product_id[$i];
					$G_DB_CONNECT->query_insert(TB_TRADING_FUTURE_TO_PRODUCT, $update_data_product_to_product);
				}
			}
			
			
				$G_DB_CONNECT->query("delete from ".TB_TRADING_FUTURE_TO_PRODUCT_TYPE." where trading_future_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_type_id) > 0 && is_array($arr_product_type_id)  && $arr_product_type_id[0] != ''){
				for($i=0;$i<count($arr_product_type_id);$i++){
					...
					$update_data_product_type_to_product_type['trading_future_id'] = $update_data['id'];
					$update_data_product_type_to_product_type['product_type_id'] = $arr_product_type_id[$i];
					$G_DB_CONNECT->query_insert(TB_TRADING_FUTURE_TO_PRODUCT_TYPE, $update_data_product_type_to_product_type);
				}
			}
			
			
				$G_DB_CONNECT->query("delete from ".TB_TRADING_FUTURE_TO_PRODUCT_CATEGORY." where trading_future_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_category_id) > 0 && is_array($arr_product_category_id)  && $arr_product_category_id[0] != ''){
				for($i=0;$i<count($arr_product_category_id);$i++){
					...
					$update_data_product_category_to_product_category['trading_future_id'] = $update_data['id'];
					$update_data_product_category_to_product_category['product_category_id'] = $arr_product_category_id[$i];
					$G_DB_CONNECT->query_insert(TB_TRADING_FUTURE_TO_PRODUCT_CATEGORY, $update_data_product_category_to_product_category);
				}
			}
			
			
			
				$G_DB_CONNECT->query("delete from ".TB_TRADING_FUTURE_TO_PRODUCT_CATEGORY2." where trading_future_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_category2_id) > 0 && is_array($arr_product_category2_id)  && $arr_product_category2_id[0] != ''){
				for($i=0;$i<count($arr_product_category2_id);$i++){
					...
					$update_data_product_category2_to_product_category2['trading_future_id'] = $update_data['id'];
					$update_data_product_category2_to_product_category2['product_category2_id'] = $arr_product_category2_id[$i];
					$G_DB_CONNECT->query_insert(TB_TRADING_FUTURE_TO_PRODUCT_CATEGORY2, $update_data_product_category2_to_product_category2);
				}
			}
			
				$G_DB_CONNECT->query("delete from ".TB_TRADING_FUTURE_TO_PRODUCT_CATEGORY3." where trading_future_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_category3_id) > 0 && is_array($arr_product_category3_id)  && $arr_product_category3_id[0] != ''){
				for($i=0;$i<count($arr_product_category3_id);$i++){
					...
					$update_data_product_category3_to_product_category3['trading_future_id'] = $update_data['id'];
					$update_data_product_category3_to_product_category3['product_category3_id'] = $arr_product_category3_id[$i];
					$G_DB_CONNECT->query_insert(TB_TRADING_FUTURE_TO_PRODUCT_CATEGORY3, $update_data_product_category3_to_product_category3);
				}
			}
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_TRADING_FUTURE," id>0 and trading_future_category_id='$trading_future_category_id' ");

...
if( $_SESSION['already_login']){
	...
	$update_data['news_category_id'] = getRequestVar('news_category_id','');
...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select news.* from ".TB_NEWS." as news, ".TB_NEWS_DESC." as news_desc ";
			$sql .= " where ";
			$sql .= " news_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and news_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and news_desc.news_id=news.id ";
			$sql .= " and news.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " news.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_NEWS, $update_data); 
			$update_data_lang['news_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_NEWS, $update_data_lang);
...
/*
			...
			$sql .= " and news_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and news_desc.news_id=news.id ";
			$sql .= " and news.id<>'".$update_data['id']."'  ";
			$sql .= " and news.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_NEWS, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['news_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_NEWS, $update_data_lang, "  news_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_NEWS,$update_data['id'],true,'');
	...
	$G_DB_CONNECT->query("delete from ".TB_NEWS_TO_PRODUCT." where news_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_id) > 0 && is_array($arr_product_id)  && $arr_product_id[0] != ''){
				for($i=0;$i<count($arr_product_id);$i++){
					...
					$update_data_product_to_product['news_id'] = $update_data['id'];
					$update_data_product_to_product['product_id'] = $arr_product_id[$i];
					$G_DB_CONNECT->query_insert(TB_NEWS_TO_PRODUCT, $update_data_product_to_product);
				}
			}
			
			
				$G_DB_CONNECT->query("delete from ".TB_NEWS_TO_PRODUCT_TYPE." where news_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_type_id) > 0 && is_array($arr_product_type_id)  && $arr_product_type_id[0] != ''){
				for($i=0;$i<count($arr_product_type_id);$i++){
					...
					$update_data_product_type_to_product_type['news_id'] = $update_data['id'];
					$update_data_product_type_to_product_type['product_type_id'] = $arr_product_type_id[$i];
					$G_DB_CONNECT->query_insert(TB_NEWS_TO_PRODUCT_TYPE, $update_data_product_type_to_product_type);
				}
			}
			
			
				$G_DB_CONNECT->query("delete from ".TB_NEWS_TO_PRODUCT_CATEGORY." where news_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_category_id) > 0 && is_array($arr_product_category_id)  && $arr_product_category_id[0] != ''){
				for($i=0;$i<count($arr_product_category_id);$i++){
					...
					$update_data_product_category_to_product_category['news_id'] = $update_data['id'];
					$update_data_product_category_to_product_category['product_category_id'] = $arr_product_category_id[$i];
					$G_DB_CONNECT->query_insert(TB_NEWS_TO_PRODUCT_CATEGORY, $update_data_product_category_to_product_category);
				}
			}
			
			
			
				$G_DB_CONNECT->query("delete from ".TB_NEWS_TO_PRODUCT_CATEGORY2." where news_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_category2_id) > 0 && is_array($arr_product_category2_id)  && $arr_product_category2_id[0] != ''){
				for($i=0;$i<count($arr_product_category2_id);$i++){
					...
					$update_data_product_category2_to_product_category2['news_id'] = $update_data['id'];
					$update_data_product_category2_to_product_category2['product_category2_id'] = $arr_product_category2_id[$i];
					$G_DB_CONNECT->query_insert(TB_NEWS_TO_PRODUCT_CATEGORY2, $update_data_product_category2_to_product_category2);
				}
			}
			
				$G_DB_CONNECT->query("delete from ".TB_NEWS_TO_PRODUCT_CATEGORY3." where news_id = '".$update_data['id']."'"); 
			...
			if(count($arr_product_category3_id) > 0 && is_array($arr_product_category3_id)  && $arr_product_category3_id[0] != ''){
				for($i=0;$i<count($arr_product_category3_id);$i++){
					...
					$update_data_product_category3_to_product_category3['news_id'] = $update_data['id'];
					$update_data_product_category3_to_product_category3['product_category3_id'] = $arr_product_category3_id[$i];
					$G_DB_CONNECT->query_insert(TB_NEWS_TO_PRODUCT_CATEGORY3, $update_data_product_category3_to_product_category3);
				}
			}

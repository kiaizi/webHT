...
if($arr_webpage_photo_disabled != '' && ID > 0){
	...
	for ($i=0; $i<count($arr_webpage_photo_disabled); $i++) {
		...
		if($tempFile != ''){
			...
			////////////////////////////////////////////////
			smart_resize_image( "../".$targetFile,"thumb", 200, 5000, true);
			smart_resize_image( "../".$targetFile,"img", 2048, 8000, true);
...
// upload file
/////////////////////////////////////////////
$arr_webpage_photo02_caption = getRequestVar("webpage_photo02_caption",'');
$arr_webpage_photo02_language_id = getRequestVar("webpage_photo02_language_id",'');
$arr_webpage_photo02_sort_order = getRequestVar("webpage_photo02_sort_order",'');
$arr_webpage_photo02_caption = getRequestVar("webpage_photo02_caption",'');
$arr_webpage_photo02_disabled = getRequestVar("webpage_photo02_disabled",'');
$arr_webpage_photo02_img_old_path = getRequestVar("webpage_photo02_img_old_path",'');
$arr_webpage_photo02_img2_old_path = getRequestVar("webpage_photo02_img2_old_path",'');
if($arr_webpage_photo02_disabled != '' && ID > 0){

	$webpage_id = ID;
	//$webpage_code = getDataName(TB_WEBPAGE,"code",$webpage_id);
	$sql = "delete from ".TB_WEBPAGE_PHOTO02." where webpage_id='".$webpage_id."'";
	$G_DB_CONNECT->query($sql);

	for ($i=0; $i<count($arr_webpage_photo02_disabled); $i++) {

		$webpage_photo02_img_old_path = $arr_webpage_photo02_img_old_path[$i];
		$webpage_photo02_img = $webpage_photo02_img_old_path;

		$dir = "images/slides/";
		$file_name = "webpage_photo02_img";
		////////////////////////////////////////////////
		$tempFile = $_FILES[$file_name]['tmp_name'][$i];
		if($tempFile != ''){
			$targetPath = CURRENT_DOCUMENT_ROOT . $dir ;
			$ext = get_extension($_FILES[$file_name]['name'][$i]);
			//$new_file_name = $webpage_code."_".date("YmdHis")."_".getLastID(TB_WEBPAGE_PHOTO02).$ext;
			$new_file_name = date("YmdHis")."_".getLastID(TB_WEBPAGE_PHOTO02).$ext;
  			$targetFile =  str_replace('//','/',$targetPath) .$new_file_name ;
			move_uploaded_file($tempFile,$targetFile);
			$targetFile =$dir.$new_file_name;
			////////////////////////////////////////////////

			$webpage_photo02_img  = $targetFile;
			
			////////////////////////////////////////////////
			smart_resize_image( "../".$targetFile,"thumb", 700, 5000, true);
		...
		if($webpage_photo02_img != '' || $webpage_photo02_img2 != ''){
			////////////////////////////////////////////////
			$update_webpage_photo02_data = array();
			$update_webpage_photo02_data['language_id'] = $arr_webpage_photo02_language_id[$i];
			$update_webpage_photo02_data['img'] = $webpage_photo02_img;
			$update_webpage_photo02_data['caption'] = $arr_webpage_photo02_caption[$i];
			//$update_webpage_photo02_data['img2'] = $webpage_photo02_img2;
			$update_webpage_photo02_data['caption'] = $arr_webpage_photo02_caption[$i];
			$update_webpage_photo02_data['sort_order'] = $arr_webpage_photo02_sort_order[$i];
			$update_webpage_photo02_data['disabled'] = $arr_webpage_photo02_disabled[$i];
			$update_webpage_photo02_data['webpage_id'] = $webpage_id;
			$update_webpage_photo02_data['create_date'] = 'null';
			$update_webpage_photo02_data['create_by'] = '';
			$update_webpage_photo02_data['last_update_by'] = '';
			$G_DB_CONNECT->query_insert(TB_WEBPAGE_PHOTO02, $update_webpage_photo02_data);
			////////////////////////////////////////////////
...
<input name="parent_webpage_id" id="parent_webpage_id" value="<?php echo SID ;?>" type="hidden"/>
<?php
...
<?php 
   	...
   	//printLangInputField(TITLE_TITLE,"name",TB_WEBPAGE,$edit_data['id'],"name",true,'','',true);
...
<tr>
    ...
    <?php
		if(ACTION == '1'){
			$edit_data['sort_order'] = getNextSortOrder(TB_WEBPAGE," parent_webpage_id='".SID."'    ");
		}
...
<?php 
	//echo printLangTextArea(TITLE_DESCRIPTION,"detail",TB_PRODUCT,$edit_data['id'],false); 
	


	printLangTextArea(TITLE_CONTENT,"detail",TB_WEBPAGE,$edit_data['id'],false,1300,1080);
...
<?php
	...
	//printLangTextAreaNormal("Left Image (Text)","name2",TB_WEBPAGE,$edit_data['id'],false,100,490);
...
<table border="0" cellspacing="2" cellpadding="0"  id="table_text2">
  ...
  <tr>
    ...
    <?php echo NOTICE_BEST_VIEW_PART_1; ?><?php echo TITLE_WIDTH; ?> : 370, <?php echo TITLE_HEIGHT; ?> : 200 <?php echo NOTICE_BEST_VIEW_PART_2; ?>
...
<tr style="display:none">
	<td class="sign_must_enter"></td>
	<td class="title vtop"  style="padding-top:12px;">Slides</td>
<td style="padding-top:0px;">
...
<?php
	$this_dynamic_area_id = "dynamic_webpage_photo02";
?>
...
<input name="webpage_photo02_img" id="webpage_photo02_img"  value="" label="<?php echo TITLE_IMG;?>" type="file" class="input_file file_upload"/>
<input name="webpage_photo02_img_old_path"  id="webpage_photo02_img_old_path" value="" type="hidden"/>
    </td>
    
    
    <td><input name="webpage_photo02_caption" id="webpage_photo02_caption"  value="" label="" class="input_short"  style="width:200px;" /></td>
...
<?php 
	...
	printCustomMenu('webpage_photo02_language_id', $sql,'name','0','','','','','');
?>
    
    </td>
    
   <td><input name="webpage_photo02_sort_order" id="webpage_photo02_sort_order"  value="" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
  ...
  $sql = "select status_disable.*, status_disable_desc.name as name from ".TB_STATUS_DISABLE." as status_disable , ".TB_STATUS_DISABLE_DESC." as status_disable_desc ";
	...
	printCustomMenu('webpage_photo02_disabled', $sql,'name','0','','','','','');
?>
    ...
    <td style="width:250px;"><a href="#"  class="button btn_remove_webpage_photo02"  ><span><?php echo BTN_REMOVE;?></span></a></td>
  </tr>
...
<?php
	...
	$i = 0;
  	$sql = "select * from ".TB_WEBPAGE_PHOTO02." as webpage_photo02  ";
	$sql .= " where webpage_id='".ID."' ";
	$sql .= " order by language_id asc,sort_order asc ";
	$rows = $G_DB_CONNECT->query($sql);
	if($G_DB_CONNECT->affected_rows > 0){
			while($data = $G_DB_CONNECT->fetch_array($rows)){
				...
				$webpage_photo02_img = "../".$data['img'];
				$arr_file_info = getImageInfo($webpage_photo02_img,"thumb",$photo02_width,$photo02_height);
				$webpage_photo02_img_thumb = $arr_file_info["src"];
...
<a href="<?php echo $webpage_photo02_img; ?>" target="_blank"><img src="<?php echo $webpage_photo02_img_thumb?>"  style="margin-bottom:10px;" align="middle" width="<?php echo $arr_file_info['width'] ?>" height="<?php echo $arr_file_info['height'] ?>"/></a>

</div>
</div>
<br>
<input name="webpage_photo02_img[]" id="webpage_photo02_img[]"  value="" label="<?php echo TITLE_PHOTO02;?>" type="file" class="input_file img_upload"/>
<input name="webpage_photo02_img_old_path[]"  id="webpage_photo02_img_old_path[]" value="<?php echo $data['img']; ?>" type="hidden"/>
    </td>
    
        <td><input name="webpage_photo02_caption[]" id="webpage_photo02_caption[]"  value="<?php echo $data['caption']; ?>" label="" class="input_short"  style="width:200px;" /></td>
...
<?php 
	...
	printCustomMenu('webpage_photo02_language_id[]', $sql,'name','0',$data['language_id'],'','','','');
?>
...
<td  ><input name="webpage_photo02_sort_order[]" id="webpage_photo02_sort_order[]"  value="<?php echo $data['sort_order']; ?>" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
	...
	printCustomMenu('webpage_photo02_disabled[]', $sql,'name','0',$data['disabled'],'','','','');
?>
...
?>
  ...
  <tr id="dynamic_webpage_photo02_footer" style="display:none">
   <td colspan="5">&nbsp;</td>
  </tr>
...
<div style="float:left;"><a href="#" id="btn_add_webpage_photo02" class="button btn_add_webpage_photo02" ><span><?php echo BTN_ADD_PHOTO;?></span></a></div>

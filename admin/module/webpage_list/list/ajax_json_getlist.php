<?php 
	...
	if($_REQUEST['update_sort_order'] > 0 && $_REQUEST['update_sort_order_data_id'] > 0){
		...
		$sql = " update ".TB_WEBPAGE;
		$sql .= " set ";
		$sql .= " sort_order = '".$_REQUEST['update_sort_order']."' ";
		$sql .= " where ";
		$sql .= " id = '".$_REQUEST['update_sort_order_data_id']."' ";
		$G_DB_CONNECT->query($sql);
		renewSortOrder(TB_WEBPAGE," parent_webpage_id='".$_REQUEST['update_sort_order_category_id']."'");
		//renewSortOrder(TB_WEBPAGE," id> 0 ");
	...
	//$return_data['table_list_content'] .= ' <th><a href="#" class="orderby_title" orderby="parent_category_name"><span>'.TITLE_PARENT_CATEGORY.'</span></a></th>';
   	$return_data['table_list_content'] .= ' <th><a href="#" class="orderby_title" orderby="nav_link"><span>'.TITLE_TITLE.'</span></a></th>';
   	$return_data['table_list_content'] .= ' <th><a href="#" class="orderby_title" orderby="nav_link_sort_order"><span>'.TITLE_SORT_ORDER.'</span></a></th>';
	
	//$return_data['table_list_content'] .= ' <th><a href="#" class="orderby_title" orderby="webpage_position_id"><span>'.TITLE_TITLE_POSITION.'</span></a></th>';
 ...
 $sql = "select webpage.*,webpage_desc.name as name from ".TB_WEBPAGE." as webpage , ".TB_WEBPAGE_DESC." as webpage_desc ";
	...
	$sql .= " and webpage.parent_webpage_id='".SID."' ";
	////////////////////////////////////////////////////////////
    ...
    $return_data['table_list_content'] .= '<td style="text-align:center; vertical-align:middle;width:100px">';
	$return_data['table_list_content'] .= getFirstPhoto(TB_WEBPAGE_PHOTO," webpage_id='".$data['id']."' ","thumb",100,200);
...
$return_data['table_list_content'] .= '<td>';
	...
	$return_data['table_list_content'] .= printSortOrderInput($data['id'],$data['sort_order'],$data['parent_webpage_id']);

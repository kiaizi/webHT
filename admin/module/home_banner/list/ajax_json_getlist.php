<?php 
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and home_banner.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_HOME_BANNER,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_HOME_BANNER,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_HOME_BANNER,$record_id,true,'');
		}
		generateHTACCESS();
	}
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select home_banner.*,home_banner_desc.name as name from ".TB_HOME_BANNER." as home_banner, ".TB_HOME_BANNER_DESC." as home_banner_desc ";
	$sql .= " where ";
	$sql .= " home_banner.id = home_banner_desc.home_banner_id ";
	$sql .= " and home_banner.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and home_banner_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by home_banner.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_HOME_BANNER_PHOTO," home_banner_id='".$data['id']."' ","thumb",200,1000);

...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_download_category_id'] != '-1'){
			$search_condition .= " and download.download_category_id = '".$_REQUEST['search_download_category_id']."' ";
		}

/*
				if($_REQUEST['search_product_id'] != '-1'){
			
			$search_condition .= " and download.id in (select download_id from ".TB_DOWNLOAD_TO_PRODUCT." where product_id='".$_REQUEST['search_product_id']."' )";
		}
		if($_REQUEST['search_product_type_id'] != '-1'){
			
			$search_condition .= " and download.id in (select download_id from ".TB_DOWNLOAD_TO_PRODUCT_TYPE." where product_type_id='".$_REQUEST['search_product_type_id']."' )";
		}
		
				if($_REQUEST['search_product_category_id'] != '-1'){
			
			$search_condition .= " and download.id in (select download_id from ".TB_DOWNLOAD_TO_PRODUCT_CATEGORY." where product_category_id='".$_REQUEST['search_product_category_id']."' )";
		}
		
				if($_REQUEST['search_product_category2_id'] != '-1'){
			
			$search_condition .= " and download.id in (select download_id from ".TB_DOWNLOAD_TO_PRODUCT_CATEGORY2." where product_category2_id='".$_REQUEST['search_product_category2_id']."' )";
		}
				if($_REQUEST['search_product_category3_id'] != '-1'){
			
			$search_condition .= " and download.id in (select download_id from ".TB_DOWNLOAD_TO_PRODUCT_CATEGORY3." where product_category3_id='".$_REQUEST['search_product_category3_id']."' )";
		}
		*/
	}
    ...
    $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="name"><span>'.TITLE_TITLE.'</span></a></th>';
	 ...
	 //   $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="download_category_id"><span>类别</span></a></th>';
...
/*
	...
	$return_data['table_list_content'] .=  getLangName(TB_DOWNLOAD_CATEGORY,"name",$data['download_category_id'],CURRENT_LANG);
		$return_data['table_list_content'] .= '</td>';
	*/

...
// upload file
/////////////////////////////////////////////
$arr_faq_photo_language_id = getRequestVar("faq_photo_language_id",'');
$arr_faq_photo_sort_order = getRequestVar("faq_photo_sort_order",'');
$arr_faq_photo_caption = getRequestVar("faq_photo_caption",'');
$arr_faq_photo_disabled = getRequestVar("faq_photo_disabled",'');
$arr_faq_photo_img_old_path = getRequestVar("faq_photo_img_old_path",'');
$arr_faq_photo_thumb_old_path = getRequestVar("faq_photo_thumb_old_path",'');
if($arr_faq_photo_disabled != '' && ID > 0){

	$faq_id = ID;
	//$faq_code = getDataName(TB_FAQ,"code",$faq_id);
	$sql = "delete from ".TB_FAQ_PHOTO." where faq_id='".$faq_id."'";
	$G_DB_CONNECT->query($sql);

	for ($i=0; $i<count($arr_faq_photo_disabled); $i++) {




		$faq_photo_img_old_path = $arr_faq_photo_img_old_path[$i];
		$faq_photo_img = $faq_photo_img_old_path;

		$dir = "upload/images/faq/";
		$file_name = "faq_photo_img";
		////////////////////////////////////////////////
		$tempFile = $_FILES[$file_name]['tmp_name'][$i];
		if($tempFile != ''){
			$targetPath = CURRENT_DOCUMENT_ROOT . $dir ;
			$ext = get_extension($_FILES[$file_name]['name'][$i]);
			//$new_file_name = $faq_code."_".date("YmdHis")."_".getLastID(TB_FAQ_PHOTO).$ext;
			$new_file_name = date("YmdHis")."_".getLastID(TB_FAQ_PHOTO).$ext;
  			$targetFile =  str_replace('//','/',$targetPath) .$new_file_name ;
			move_uploaded_file($tempFile,$targetFile);
			$targetFile =$dir.$new_file_name;
			////////////////////////////////////////////////

			$faq_photo_img  = $targetFile;
		...
		if($faq_photo_img != '' ){
			////////////////////////////////////////////////
			$update_faq_photo_data = array();
			$update_faq_photo_data['language_id'] = $arr_faq_photo_language_id[$i];
			$update_faq_photo_data['img'] = $faq_photo_img;
			$update_faq_photo_data['thumb'] = $faq_photo_thumb;
			$update_faq_photo_data['caption'] = $arr_faq_photo_caption[$i];
			$update_faq_photo_data['sort_order'] = $arr_faq_photo_sort_order[$i];
			$update_faq_photo_data['disabled'] = $arr_faq_photo_disabled[$i];
			$update_faq_photo_data['faq_id'] = $faq_id;
			$update_faq_photo_data['create_date'] = 'null';
			$update_faq_photo_data['create_by'] = '';
			$update_faq_photo_data['last_update_by'] = '';
			$G_DB_CONNECT->query_insert(TB_FAQ_PHOTO, $update_faq_photo_data);
			////////////////////////////////////////////////
...
echo $G_DB_CONNECT->query_update_lang_content(TB_FAQ, $update_data_lang, "  faq_id='".$update_data['id']."'   ");
*/
...
<?php 
   	...
   	printLangInputField("SEO H1","seo_h1",TB_FAQ,$edit_data['id'],"seo_h1",false,"input_seo_title","",false,'class="seo_row"');
...
<?php 
   	...
   	printLangInputField(TITLE_SEO_TITLE,"seo_title",TB_FAQ,$edit_data['id'],"seo_title",false,"input_seo_title","",false,'class="seo_row"');
...
printLangTextAreaNormal(TITLE_SEO_DESC,"seo_desc",TB_FAQ,$edit_data['id'],false,300,450,'','class="seo_row"');
?>
...
<tr>
    ...
    <?php
		if(ACTION == '1'){
			$edit_data['sort_order'] = getNextSortOrder(TB_FAQ,"");
		}
	?>
...
<tr style="display:none">
    ...
    <?php
		if(ACTION == '1'){
			//$edit_data['code'] = nextRecordcode(TB_FAQ,"","",4);
		}
	?>
...
<?php 
   ...
   $sql = "select faq_category.*, faq_category_desc.name as name from ".TB_FAQ_CATEGORY." as faq_category , ".TB_FAQ_CATEGORY_DESC." as faq_category_desc ";
	...
	$sql .= " and faq_category_desc.language_id='".ADMIN_LANG_ID."' and disabled<>'".DISABLED_DELETE."' ";
	$sql .= " order by faq_category.sort_order asc ";
				

	printCustomMenu('faq_category_id', $sql,'name','0',$edit_data['faq_category_id'],'','','','');
...
<?php 
   	...
   	printLangInputField(TITLE_TITLE,"name",TB_FAQ,$edit_data['id'],"name",true);
	//printLangTextAreaNormal('Brief',"detail2",TB_FAQ,$edit_data['id'],false,150,500,"");
	printLangTextArea(TITLE_DESC,"detail",TB_FAQ,$edit_data['id'],true,1300,850);
		

	//printLangTextArea('參考資料',"detail2",TB_FAQ,$edit_data['id'],true,500,850);
...
<?php
		...
		while($data2 = $G_DB_CONNECT->fetch_array($rows2)){
			...
			$sql .= " from ".TB_FAQ_TO_PRODUCT_TYPE;
			$sql .= " where ";
			$sql .= " faq_id = '".ID."' ";
...
<?php
		...
		while($data2 = $G_DB_CONNECT->fetch_array($rows2)){
			...
			$sql .= " from ".TB_FAQ_TO_PRODUCT_CATEGORY;
			$sql .= " where ";
			$sql .= " faq_id = '".ID."' ";
...
<?php
		...
		while($data2 = $G_DB_CONNECT->fetch_array($rows2)){
			...
			$sql .= " from ".TB_FAQ_TO_PRODUCT_CATEGORY2;
			$sql .= " where ";
			$sql .= " faq_id = '".ID."' ";
...
<?php
		...
		while($data2 = $G_DB_CONNECT->fetch_array($rows2)){
			...
			$sql .= " from ".TB_FAQ_TO_PRODUCT_CATEGORY3;
			$sql .= " where ";
			$sql .= " faq_id = '".ID."' ";
...
?>
 ...
 <?php //echo printLangTextArea(TITLE_CONTENT_IF_NO_LINK,"detail",TB_FAQ,$edit_data['id'],false,800); ?>  

   
<?php
// printLangTextAreaNormal(TITLE_TITLE,"name",TB_FAQ,$edit_data['id'],false,150,500,"");
?>
...
<?php
 //printLangTextAreaNormal(TITLE_ADDRESS,"address",TB_FAQ,$edit_data['id'],false,150,500,"");
?>
...
<?php
	$this_dynamic_area_id = "dynamic_faq_photo";
?>
...
<input name="faq_photo_img" id="faq_photo_img"  value="" label="<?php echo TITLE_IMG;?>" type="file" class="input_file file_upload"/>
<input name="faq_photo_img_old_path"  id="faq_photo_img_old_path" value="" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('faq_photo_language_id', $sql,'name','0','','','','','');
?>
    
    </td>
    
<td><input name="faq_photo_sort_order" id="faq_photo_sort_order"  value="" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
  ...
  $sql = "select status_disable.*, status_disable_desc.name as name from ".TB_STATUS_DISABLE." as status_disable , ".TB_STATUS_DISABLE_DESC." as status_disable_desc ";
	...
	printCustomMenu('faq_photo_disabled', $sql,'name','0','','','','','');
?>
    ...
    <td style="width:250px;"><a href="#"  class="button btn_remove_faq_photo"   ><span><?php echo BTN_REMOVE;?></span></a></td>
  </tr>
...
<?php
	...
	$i = 0;
  	$sql = "select * from ".TB_FAQ_PHOTO." as faq_photo  ";
	$sql .= " where faq_id='".ID."' ";
	...
	if($G_DB_CONNECT->affected_rows > 0){
			while($data = $G_DB_CONNECT->fetch_array($rows)){
				...
				$faq_photo_img = "../".$data['img'];
				$arr_file_info = getImageInfo($faq_photo_img,"thumb",$photo_width,$photo_height);
				$faq_photo_img_thumb = $arr_file_info["src"];
...
<a href="<?php echo $faq_photo_img; ?>" target="_blank"><img src="<?php echo $faq_photo_img_thumb?>"  style="margin-bottom:10px;" align="middle" width="<?php echo $arr_file_info['width'] ?>" height="<?php echo $arr_file_info['height'] ?>"/></a>

</div>
</div>
<br>
<input name="faq_photo_img[]" id="faq_photo_img[]"  value="" label="<?php echo TITLE_PHOTO;?>" type="file" class="input_file img_upload"/>
<input name="faq_photo_img_old_path[]"  id="faq_photo_img_old_path[]" value="<?php echo $data['img']; ?>" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('faq_photo_language_id[]', $sql,'name','0',$data['language_id'],'','','','');
?>
   ...
   <td><input name="faq_photo_sort_order[]" id="faq_photo_sort_order[]"  value="<?php echo $data['sort_order']; ?>" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
	...
	printCustomMenu('faq_photo_disabled[]', $sql,'name','0',$data['disabled'],'','','','');
?>
...
?>
  ...
  <tr id="dynamic_faq_photo_footer" style="display:none">
   <td colspan="5">&nbsp;</td>
  </tr>
...
<div style="float:left;"><a href="#" id="btn_add_faq_photo" class="button btn_add_faq_photo"  ><span><?php echo BTN_ADD_PHOTO;?></span></a></div>

...
//////////////////////////////////////////////////////////
	if(ACTION == '1'){
		...
		$sql .= " where ";
		$sql .= "  enquiry_career.username='$username'  ";
		$sql .= " and enquiry_career.disabled<>'".DISABLED_DELETE."' ";
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_ENQUIRY_CAREER, $update_data); 
		}
	...
	if(ACTION == '2'){
		$sql = "select enquiry_career.* from ".TB_ENQUIRY_CAREER." as enquiry_career ";
		$sql .= " where ";
		$sql .= "  enquiry_career.username='$username'  and enquiry_career.id <> '".$update_data['id']."' ";
		$sql .= " and enquiry_career.disabled<>'".DISABLED_DELETE."' ";
		$sql .= " group by enquiry_career.id  ";
		$rows = $G_DB_CONNECT->query($sql);
		if($G_DB_CONNECT->affected_rows > 0){
			...
			$sql = "select enquiry_career.* from ".TB_ENQUIRY_CAREER." as enquiry_career ";
			$sql .= " where ";
			$sql .= " enquiry_career.code='$code'  and enquiry_career.id <> '".$update_data['id']."' ";
			$sql .= " and enquiry_career.disabled<>'".DISABLED_DELETE."' ";
			$sql .= " group by enquiry_career.id  ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_ENQUIRY_CAREER, $update_data, "id='".$update_data['id']."'"); 
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_ENQUIRY_CAREER,$update_data['id'],true,'');

...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select service_category.* from ".TB_SERVICE_CATEGORY." as service_category, ".TB_SERVICE_CATEGORY_DESC." as service_category_desc ";
			$sql .= " where ";
			$sql .= " service_category_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and service_category_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and service_category_desc.service_category_id=service_category.id ";
			$sql .= " and service_category.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " service_category.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_SERVICE_CATEGORY, $update_data); 
			$update_data_lang['service_category_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_SERVICE_CATEGORY, $update_data_lang);
...
/*
			...
			$sql .= " and service_category_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and service_category_desc.service_category_id=service_category.id ";
			$sql .= " and service_category.id<>'".$update_data['id']."'  ";
			$sql .= " and service_category.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_SERVICE_CATEGORY, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['service_category_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_SERVICE_CATEGORY, $update_data_lang, "  service_category_id='".$update_data['id']."'   ");
		}
	...
	if($update_data['seo_url'] == ''){
		$G_DB_CONNECT->query("update ".TB_SERVICE_CATEGORY." set seo_url='".$return_data['id']."'  where id='".$return_data['id']."' ");
	}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_SERVICE_CATEGORY,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_SERVICE_CATEGORY," id>0 ");

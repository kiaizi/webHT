...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['update_sort_order'] > 0 && $_REQUEST['update_sort_order_data_id'] > 0){
		...
		$sql = " update ".TB_SERVICE2;
		$sql .= " set ";
		$sql .= " sort_order = '".$_REQUEST['update_sort_order']."' ";
		$sql .= " where ";
		$sql .= " id = '".$_REQUEST['update_sort_order_data_id']."'";
		$G_DB_CONNECT->query($sql);
		renewSortOrder(TB_SERVICE2,"service2_category_id='".$_REQUEST['update_sort_order_category_id']."'");
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and service2.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	if($_REQUEST['search_service2_category_id'] != '-1'){
			$search_condition .= " and service2.service2_category_id = '".$_REQUEST['search_service2_category_id']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_SERVICE2,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_SERVICE2,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_SERVICE2,$record_id,true,'');
		}
		generateHTACCESS();
	}
	...
	//$return_data['table_list_content'] .= ' <th><a href="#" class="orderby_title" orderby="tel"><span>'.TITLE_TEL.'</span></a></th>';
	 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="service2_category_id"><span>'.TITLE_CATEGORY.'</span></a></th>';
 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="sort_order"><span>'.TITLE_SORT_ORDER.'</span></a></th>';
  //   $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="new_service2"><span>'.TITLE_NEW_SERVICE2.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select service2.*,service2_desc.name as name from ".TB_SERVICE2." as service2, ".TB_SERVICE2_DESC." as service2_desc ";
	$sql .= " where ";
	$sql .= " service2.id = service2_desc.service2_id ";
	$sql .= " and service2.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and service2_desc.language_id = '".ADMIN_LANG_ID."'";
	
	
	$sql .= $search_condition;
	//$sql .= " group by service2.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_SERVICE2_PHOTO," service2_id='".$data['id']."' ","thumb",100,500);
	...
	$return_data['table_list_content'] .= '<tr>';
	$return_data['table_list_content'] .= '<td style="border:none;text-align:center; vertical-align:middle;background-color:#FFFFFF">';
	$return_data['table_list_content'] .= getFirstPhoto(TB_SERVICE2_PHOTO02," service2_id='".$data['id']."' ","thumb",100,500);
	$return_data['table_list_content'] .= '</td>';
	$return_data['table_list_content'] .= '</tr>';
	$return_data['table_list_content'] .= '</table>';
...
$return_data['table_list_content'] .= '<td>'.getLangName(TB_SERVICE2_CATEGORY,"name",$data['service2_category_id'],ADMIN_LANG_ID).'</td>';
	$return_data['table_list_content'] .= '<td>';
	

	$return_data['table_list_content'] .= printSortOrderInput($data['id'],$data['sort_order'],$data['service2_category_id']);
	
	
	$return_data['table_list_content'] .= '</td>';
	
   // $return_data['table_list_content'] .= '<td>'.displayYesNo($data['new_service2']).'</td>';

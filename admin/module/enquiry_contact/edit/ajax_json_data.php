...
//////////////////////////////////////////////////////////
	if(ACTION == '1'){
		...
		$sql .= " where ";
		$sql .= "  enquiry_contact.username='$username'  ";
		$sql .= " and enquiry_contact.disabled<>'".DISABLED_DELETE."' ";
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_ENQUIRY_CONTACT, $update_data); 
		}
	...
	if(ACTION == '2'){
		$sql = "select enquiry_contact.* from ".TB_ENQUIRY_CONTACT." as enquiry_contact ";
		$sql .= " where ";
		$sql .= "  enquiry_contact.username='$username'  and enquiry_contact.id <> '".$update_data['id']."' ";
		$sql .= " and enquiry_contact.disabled<>'".DISABLED_DELETE."' ";
		$sql .= " group by enquiry_contact.id  ";
		$rows = $G_DB_CONNECT->query($sql);
		if($G_DB_CONNECT->affected_rows > 0){
			...
			$sql = "select enquiry_contact.* from ".TB_ENQUIRY_CONTACT." as enquiry_contact ";
			$sql .= " where ";
			$sql .= " enquiry_contact.code='$code'  and enquiry_contact.id <> '".$update_data['id']."' ";
			$sql .= " and enquiry_contact.disabled<>'".DISABLED_DELETE."' ";
			$sql .= " group by enquiry_contact.id  ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_ENQUIRY_CONTACT, $update_data, "id='".$update_data['id']."'"); 
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_ENQUIRY_CONTACT,$update_data['id'],true,'');

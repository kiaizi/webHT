...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select partner.* from ".TB_PARTNER." as partner, ".TB_PARTNER_DESC." as partner_desc ";
			$sql .= " where ";
			$sql .= " partner_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and partner_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and partner_desc.partner_id=partner.id ";
			$sql .= " and partner.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " partner.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_PARTNER, $update_data); 
			$update_data_lang['partner_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_PARTNER, $update_data_lang);
...
/*
			...
			$sql .= " and partner_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and partner_desc.partner_id=partner.id ";
			$sql .= " and partner.id<>'".$update_data['id']."'  ";
			$sql .= " and partner.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_PARTNER, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['partner_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_PARTNER, $update_data_lang, "  partner_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_PARTNER,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_PARTNER," id>0 ");

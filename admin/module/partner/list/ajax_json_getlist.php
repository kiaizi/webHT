...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and partner.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_PARTNER,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_PARTNER,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_PARTNER,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
//	$return_data['table_list_content'] .= '<th width="100"><a href="#" class="orderby_title" orderby="display_date"><span>'.TITLE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select partner.*,partner_desc.name as name,partner_desc.detail3 from ".TB_PARTNER." as partner, ".TB_PARTNER_DESC." as partner_desc ";
	$sql .= " where ";
	$sql .= " partner.id = partner_desc.partner_id ";
	$sql .= " and partner.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and partner_desc.language_id = '".ADMIN_LANG_ID."'";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_PARTNER_PHOTO," partner_id='".$data['id']."' ","thumb",200,100);

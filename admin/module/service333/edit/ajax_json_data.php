...
if( $_SESSION['already_login']){
	...
	$update_data['service_category_id'] = getRequestVar('service_category_id','');
	$update_data['new_service'] = getRequestVar('new_service','');
...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select service.* from ".TB_SERVICE." as service, ".TB_SERVICE_DESC." as service_desc ";
			$sql .= " where ";
			$sql .= " service_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and service_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and service_desc.service_id=service.id ";
			$sql .= " and service.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " service.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_SERVICE, $update_data); 
			$update_data_lang['service_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_SERVICE, $update_data_lang);
...
/*
			...
			$sql .= " and service_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and service_desc.service_id=service.id ";
			$sql .= " and service.id<>'".$update_data['id']."'  ";
			$sql .= " and service.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_SERVICE, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['service_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_SERVICE, $update_data_lang, "  service_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_SERVICE,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_SERVICE," id>0 and service_category_id='".$update_data['service_category_id']."' ");

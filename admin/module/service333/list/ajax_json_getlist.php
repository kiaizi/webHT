...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['update_sort_order'] > 0 && $_REQUEST['update_sort_order_data_id'] > 0){
		...
		renewSortOrder(TB_SERVICE,"service_category_id='".$_REQUEST['update_sort_order_category_id']."'");
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and service.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	if($_REQUEST['search_service_category_id'] != '-1'){
			$search_condition .= " and service.service_category_id = '".$_REQUEST['search_service_category_id']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_SERVICE,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_SERVICE,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_SERVICE,$record_id,true,'');
		}
		generateHTACCESS();
	}
	...
	//$return_data['table_list_content'] .= ' <th><a href="#" class="orderby_title" orderby="tel"><span>'.TITLE_TEL.'</span></a></th>';
	 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="service_category_id"><span>'.TITLE_CATEGORY.'</span></a></th>';
 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="sort_order"><span>'.TITLE_SORT_ORDER.'</span></a></th>';
  //   $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="new_service"><span>'.TITLE_NEW_SERVICE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select service.*,service_desc.name as name from ".TB_SERVICE." as service, ".TB_SERVICE_DESC." as service_desc ";
	$sql .= " where ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_SERVICE_PHOTO03," service_id='".$data['id']."' ","thumb",100,500);
	...
	$return_data['table_list_content'] .= '<tr>';
	$return_data['table_list_content'] .= '<td style="border:none;text-align:center; vertical-align:middle;background-color:#FFFFFF">';
	$return_data['table_list_content'] .= getFirstPhoto(TB_SERVICE_PHOTO02," service_id='".$data['id']."' ","thumb",100,500);
	$return_data['table_list_content'] .= '</td>';
	$return_data['table_list_content'] .= '</tr>';
	$return_data['table_list_content'] .= '</table>';
...
$return_data['table_list_content'] .= '<td>'.getLangName(TB_SERVICE_CATEGORY,"name",$data['service_category_id'],ADMIN_LANG_ID).'</td>';
	$return_data['table_list_content'] .= '<td>';
	

	$return_data['table_list_content'] .= printSortOrderInput($data['id'],$data['sort_order'],$data['service_category_id']);
	
	
	$return_data['table_list_content'] .= '</td>';
	
   // $return_data['table_list_content'] .= '<td>'.displayYesNo($data['new_service']).'</td>';

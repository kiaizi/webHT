...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and career.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_CAREER,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_CAREER,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_CAREER,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
//$return_data['table_list_content'] .= ' <th width="100">&nbsp;</th>';
 ...
 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="sort_order"><span>'.TITLE_SORT_ORDER.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select career.*,career_desc.name as name from ".TB_CAREER." as career, ".TB_CAREER_DESC." as career_desc ";
	$sql .= " where ";
	$sql .= " career.id = career_desc.career_id ";
	$sql .= " and career.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and career_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by career.id  ";
...
/*
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_CAREER_PHOTO," career_id='".$data['id']."' ","thumb",100,500);

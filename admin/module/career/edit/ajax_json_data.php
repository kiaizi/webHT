...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select career.* from ".TB_CAREER." as career, ".TB_CAREER_DESC." as career_desc ";
			$sql .= " where ";
			$sql .= " career_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and career_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and career_desc.career_id=career.id ";
			$sql .= " and career.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " career.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_CAREER, $update_data); 
			$update_data_lang['career_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_CAREER, $update_data_lang);
...
/*
			...
			$sql .= " and career_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and career_desc.career_id=career.id ";
			$sql .= " and career.id<>'".$update_data['id']."'  ";
			$sql .= " and career.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_CAREER, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['career_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_CAREER, $update_data_lang, "  career_id='".$update_data['id']."'   ");
		}
	...
	if($update_data['seo_url'] == ''){
		$G_DB_CONNECT->query("update ".TB_CAREER." set seo_url='".$return_data['id']."'  where id='".$return_data['id']."' ");
	}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_CAREER,$update_data['id'],true,'');

...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select aboutfeature.* from ".TB_ABOUTFEATURE." as aboutfeature, ".TB_ABOUTFEATURE_DESC." as aboutfeature_desc ";
			$sql .= " where ";
			$sql .= " aboutfeature_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and aboutfeature_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and aboutfeature_desc.aboutfeature_id=aboutfeature.id ";
			$sql .= " and aboutfeature.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " aboutfeature.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_ABOUTFEATURE, $update_data); 
			$update_data_lang['aboutfeature_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_ABOUTFEATURE, $update_data_lang);
...
/*
			...
			$sql .= " and aboutfeature_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and aboutfeature_desc.aboutfeature_id=aboutfeature.id ";
			$sql .= " and aboutfeature.id<>'".$update_data['id']."'  ";
			$sql .= " and aboutfeature.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_ABOUTFEATURE, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['aboutfeature_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_ABOUTFEATURE, $update_data_lang, "  aboutfeature_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_ABOUTFEATURE,$update_data['id'],true,'');

...
//////////////////////////////////////////////////////////
	if(ACTION == '1'){
		...
		$sql .= " where ";
		$sql .= "  enquiry2.username='$username'  ";
		$sql .= " and enquiry2.disabled<>'".DISABLED_DELETE."' ";
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_ENQUIRY2, $update_data); 
		}
	...
	if(ACTION == '2'){
		$sql = "select enquiry2.* from ".TB_ENQUIRY2." as enquiry2 ";
		$sql .= " where ";
		$sql .= "  enquiry2.username='$username'  and enquiry2.id <> '".$update_data['id']."' ";
		$sql .= " and enquiry2.disabled<>'".DISABLED_DELETE."' ";
		$sql .= " group by enquiry2.id  ";
		$rows = $G_DB_CONNECT->query($sql);
		if($G_DB_CONNECT->affected_rows > 0){
			...
			$sql = "select enquiry2.* from ".TB_ENQUIRY2." as enquiry2 ";
			$sql .= " where ";
			$sql .= " enquiry2.code='$code'  and enquiry2.id <> '".$update_data['id']."' ";
			$sql .= " and enquiry2.disabled<>'".DISABLED_DELETE."' ";
			$sql .= " group by enquiry2.id  ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_ENQUIRY2, $update_data, "id='".$update_data['id']."'"); 
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_ENQUIRY2,$update_data['id'],true,'');

...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and form_option3.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_FORM_OPTION3,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_FORM_OPTION3,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_FORM_OPTION3,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
//$return_data['table_list_content'] .= ' <th width="100">&nbsp;</th>';
 ...
 $return_data['table_list_content'] .= '<th><a href="#" class="orderby_title" orderby="sort_order"><span>'.TITLE_SORT_ORDER.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select form_option3.*,form_option3_desc.name as name from ".TB_FORM_OPTION3." as form_option3, ".TB_FORM_OPTION3_DESC." as form_option3_desc ";
	$sql .= " where ";
	$sql .= " form_option3.id = form_option3_desc.form_option3_id ";
	$sql .= " and form_option3.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and form_option3_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by form_option3.id  ";
...
/*
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_FORM_OPTION3_PHOTO," form_option3_id='".$data['id']."' ","thumb",100,500);
	...
	$return_data['table_list_content'] .= '<tr>';
	$return_data['table_list_content'] .= '<td style="border:none;text-align:center; vertical-align:middle;background-color:#FFFFFF">';
	$return_data['table_list_content'] .= getFirstPhoto(TB_FORM_OPTION3_PHOTO02," form_option3_id='".$data['id']."' ","thumb",100,500);
	$return_data['table_list_content'] .= '</td>';
	$return_data['table_list_content'] .= '</tr>';
	$return_data['table_list_content'] .= '</table>';
	...
	$return_data['table_list_content'] .= '<tr>';
	$return_data['table_list_content'] .= '<td style="border:none;text-align:center; vertical-align:middle;background-color:#FFFFFF">';
	$return_data['table_list_content'] .= getFirstPhoto(TB_FORM_OPTION3_PHOTO03," form_option3_id='".$data['id']."' ","thumb",100,500);
	$return_data['table_list_content'] .= '</td>';
	$return_data['table_list_content'] .= '</tr>';
	$return_data['table_list_content'] .= '</table>';

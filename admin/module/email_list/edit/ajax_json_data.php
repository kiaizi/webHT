...
//////////////////////////////////////////////////////////
	if(ACTION == '1'){
		...
		$sql .= " where ";
		$sql .= "  email_list.username='$username'  ";
		$sql .= " and email_list.disabled<>'".DISABLED_DELETE."' ";
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_EMAIL_LIST, $update_data); 
		}
	...
	if(ACTION == '2'){
		$sql = "select email_list.* from ".TB_EMAIL_LIST." as email_list ";
		$sql .= " where ";
		$sql .= "  email_list.username='$username'  and email_list.id <> '".$update_data['id']."' ";
		$sql .= " and email_list.disabled<>'".DISABLED_DELETE."' ";
		$sql .= " group by email_list.id  ";
		$rows = $G_DB_CONNECT->query($sql);
		if($G_DB_CONNECT->affected_rows > 0){
			...
			$sql = "select email_list.* from ".TB_EMAIL_LIST." as email_list ";
			$sql .= " where ";
			$sql .= " email_list.code='$code'  and email_list.id <> '".$update_data['id']."' ";
			$sql .= " and email_list.disabled<>'".DISABLED_DELETE."' ";
			$sql .= " group by email_list.id  ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_EMAIL_LIST, $update_data, "id='".$update_data['id']."'"); 
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_EMAIL_LIST,$update_data['id'],true,'');

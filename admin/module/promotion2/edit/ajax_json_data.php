...
//////////////////////////////////////////////////////////
	...
	if(ACTION == '1'){
		/*
			$sql = "select promotion2.* from ".TB_PROMOTION2." as promotion2, ".TB_PROMOTION2_DESC." as promotion2_desc ";
			$sql .= " where ";
			$sql .= " promotion2_desc.name='".getRequestVar('name_'.DEFAULT_FRONT_LANG_ID,'')."'  ";
			$sql .= " and promotion2_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and promotion2_desc.promotion2_id=promotion2.id ";
			$sql .= " and promotion2.disabled <> '".DISABLED_DELETE."' ";
			//$sql .= " promotion2.id<>'".$update_data['id']."'  ";
		...
		*/
		...
		if($return_data['success']){
			...
			$return_data['id'] = $update_data['id'] = $G_DB_CONNECT->query_insert(TB_PROMOTION2, $update_data); 
			$update_data_lang['promotion2_id'] = $return_data['id'];
			$G_DB_CONNECT->query_insert_lang_content(TB_PROMOTION2, $update_data_lang);
...
/*
			...
			$sql .= " and promotion2_desc.language_id='".DEFAULT_FRONT_LANG_ID."'  ";
			$sql .= " and promotion2_desc.promotion2_id=promotion2.id ";
			$sql .= " and promotion2.id<>'".$update_data['id']."'  ";
			$sql .= " and promotion2.disabled <> '".DISABLED_DELETE."' ";
			$rows = $G_DB_CONNECT->query($sql);
			if($G_DB_CONNECT->affected_rows > 0){
				$return_data['warn_msg'] = TITLE_TITLE.WARN_EXIST ;
		...
		*/
		...
		//////////////////////////////////////////////////////////
		if($return_data['success']){
			$G_DB_CONNECT->query_update(TB_PROMOTION2, $update_data, "id='".$update_data['id']."'"); 
			$update_data_lang['promotion2_id'] = $return_data['id'];
			$G_DB_CONNECT->query_update_lang_content(TB_PROMOTION2, $update_data_lang, "  promotion2_id='".$update_data['id']."'   ");
		}
	...
	//////////////////////////////////////////////////////////
	if(ACTION == '3'){
			deleteRecord(TB_PROMOTION2,$update_data['id'],true,'');
	...
	//////////////////////////////////////////////////////////
  	...
  	renewSortOrder(TB_PROMOTION2," id>0 ");

<?php 
	...
	$sql = "select * from ".TB_PROMOTION2." as promotion2 ";
	$sql .= " where ";
	$sql .= " id='".ID."' ";
...
// upload file
/////////////////////////////////////////////
$arr_promotion2_photo_language_id = getRequestVar("promotion2_photo_language_id",'');
$arr_promotion2_photo_sort_order = getRequestVar("promotion2_photo_sort_order",'');
$arr_promotion2_photo_caption = getRequestVar("promotion2_photo_caption",'');
$arr_promotion2_photo_disabled = getRequestVar("promotion2_photo_disabled",'');
$arr_promotion2_photo_img_old_path = getRequestVar("promotion2_photo_img_old_path",'');
$arr_promotion2_photo_thumb_old_path = getRequestVar("promotion2_photo_thumb_old_path",'');
if($arr_promotion2_photo_disabled != '' && ID > 0){

	$promotion2_id = ID;
	//$promotion2_code = getDataName(TB_PROMOTION2,"code",$promotion2_id);
	$sql = "delete from ".TB_PROMOTION2_PHOTO." where promotion2_id='".$promotion2_id."'";
	$G_DB_CONNECT->query($sql);

	for ($i=0; $i<count($arr_promotion2_photo_disabled); $i++) {




		$promotion2_photo_img_old_path = $arr_promotion2_photo_img_old_path[$i];
		$promotion2_photo_img = $promotion2_photo_img_old_path;

		$dir = "upload/images/promotion2/";
		$file_name = "promotion2_photo_img";
		////////////////////////////////////////////////
		$tempFile = $_FILES[$file_name]['tmp_name'][$i];
		if($tempFile != ''){
			$targetPath = CURRENT_DOCUMENT_ROOT . $dir ;
			$ext = get_extension($_FILES[$file_name]['name'][$i]);
			//$new_file_name = $promotion2_code."_".date("YmdHis")."_".getLastID(TB_PROMOTION2_PHOTO).$ext;
			$new_file_name = date("YmdHis")."_".getLastID(TB_PROMOTION2_PHOTO).$ext;
  			$targetFile =  str_replace('//','/',$targetPath) .$new_file_name ;
			move_uploaded_file($tempFile,$targetFile);
			$targetFile =$dir.$new_file_name;
			////////////////////////////////////////////////

			$promotion2_photo_img  = $targetFile;
		...
		if($promotion2_photo_img != '' ){
			////////////////////////////////////////////////
			$update_promotion2_photo_data = array();
			$update_promotion2_photo_data['language_id'] = $arr_promotion2_photo_language_id[$i];
			$update_promotion2_photo_data['img'] = $promotion2_photo_img;
			$update_promotion2_photo_data['thumb'] = $promotion2_photo_thumb;
			$update_promotion2_photo_data['caption'] = $arr_promotion2_photo_caption[$i];
			$update_promotion2_photo_data['sort_order'] = $arr_promotion2_photo_sort_order[$i];
			$update_promotion2_photo_data['disabled'] = $arr_promotion2_photo_disabled[$i];
			$update_promotion2_photo_data['promotion2_id'] = $promotion2_id;
			$update_promotion2_photo_data['create_date'] = 'null';
			$update_promotion2_photo_data['create_by'] = '';
			$update_promotion2_photo_data['last_update_by'] = '';
			$G_DB_CONNECT->query_insert(TB_PROMOTION2_PHOTO, $update_promotion2_photo_data);
			////////////////////////////////////////////////
...
echo $G_DB_CONNECT->query_update_lang_content(TB_PROMOTION2, $update_data_lang, "  promotion2_id='".$update_data['id']."'   ");
*/
...
<?php 
   	...
   	printLangInputField(TITLE_SEO_TITLE,"seo_title",TB_PROMOTION2,$edit_data['id'],"seo_title",false,"input_seo_title","",false,'class="seo_row"');
...
printLangTextAreaNormal(TITLE_SEO_DESC,"seo_desc",TB_PROMOTION2,$edit_data['id'],false,300,450,'','class="seo_row"');
?>
...
<tr>
    ...
    <?php
		if(ACTION == '1'){
			$edit_data['sort_order'] = getNextSortOrder(TB_PROMOTION2,"");
		}
	?>
...
<tr style="display:none">
    ...
    <?php
		if(ACTION == '1'){
			//$edit_data['code'] = nextRecordcode(TB_PROMOTION2,"","",4);
		}
	?>
...
<?php 
   	...
   	printLangInputField(TITLE_TITLE,"name",TB_PROMOTION2,$edit_data['id'],"name",true);
	//printLangTextAreaNormal('Brief',"detail2",TB_PROMOTION2,$edit_data['id'],false,150,500,"");
	printLangTextArea(TITLE_DESC,"detail",TB_PROMOTION2,$edit_data['id'],true,800,850);
...
?>
 ...
 <?php //echo printLangTextArea(TITLE_CONTENT_IF_NO_LINK,"detail",TB_PROMOTION2,$edit_data['id'],false,800); ?>  

   
<?php
// printLangTextAreaNormal(TITLE_TITLE,"name",TB_PROMOTION2,$edit_data['id'],false,150,500,"");
?>
...
<?php
 //printLangTextAreaNormal(TITLE_ADDRESS,"address",TB_PROMOTION2,$edit_data['id'],false,150,500,"");
?>
...
<?php
	$this_dynamic_area_id = "dynamic_promotion2_photo";
?>
...
<input name="promotion2_photo_img" id="promotion2_photo_img"  value="" label="<?php echo TITLE_IMG;?>" type="file" class="input_file file_upload"/>
<input name="promotion2_photo_img_old_path"  id="promotion2_photo_img_old_path" value="" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('promotion2_photo_language_id', $sql,'name','0','','','','','');
?>
    
    </td>
    
<td><input name="promotion2_photo_sort_order" id="promotion2_photo_sort_order"  value="" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
  ...
  $sql = "select status_disable.*, status_disable_desc.name as name from ".TB_STATUS_DISABLE." as status_disable , ".TB_STATUS_DISABLE_DESC." as status_disable_desc ";
	...
	printCustomMenu('promotion2_photo_disabled', $sql,'name','0','','','','','');
?>
    ...
    <td style="width:250px;"><a href="#"  class="button btn_remove_promotion2_photo"   ><span><?php echo BTN_REMOVE;?></span></a></td>
  </tr>
...
<?php
	...
	$i = 0;
  	$sql = "select * from ".TB_PROMOTION2_PHOTO." as promotion2_photo  ";
	$sql .= " where promotion2_id='".ID."' ";
	...
	if($G_DB_CONNECT->affected_rows > 0){
			while($data = $G_DB_CONNECT->fetch_array($rows)){
				...
				$promotion2_photo_img = "../".$data['img'];
				$arr_file_info = getImageInfo($promotion2_photo_img,"thumb",$photo_width,$photo_height);
				$promotion2_photo_img_thumb = $arr_file_info["src"];
...
<?php
	...
	$photo_height = 70;
				...
				$promotion2_photo_img = "../".$data['thumb'];
				$arr_file_info = getImageInfo($promotion2_photo_img,"thumb",$photo_width,$photo_height);
				$promotion2_photo_img_thumb = $arr_file_info["src"];
...
<a href="<?php echo $promotion2_photo_img; ?>" target="_blank"><img src="<?php echo $promotion2_photo_img_thumb?>"  style="margin-bottom:10px;" align="middle" width="<?php echo $arr_file_info['width'] ?>" height="<?php echo $arr_file_info['height'] ?>"/></a>

</div>
</div>
<br>
<input name="promotion2_photo_img[]" id="promotion2_photo_img[]"  value="" label="<?php echo TITLE_PHOTO;?>" type="file" class="input_file img_upload"/>
<input name="promotion2_photo_img_old_path[]"  id="promotion2_photo_img_old_path[]" value="<?php echo $data['img']; ?>" type="hidden"/>
    </td>
...
<?php 
	...
	printCustomMenu('promotion2_photo_language_id[]', $sql,'name','0',$data['language_id'],'','','','');
?>
   ...
   <td><input name="promotion2_photo_sort_order[]" id="promotion2_photo_sort_order[]"  value="<?php echo $data['sort_order']; ?>" label="<?php echo TITLE_SORT_ORDER;?>" class="input_short sort_order"   maxlength="5"/></td>
...
<?php 
	...
	printCustomMenu('promotion2_photo_disabled[]', $sql,'name','0',$data['disabled'],'','','','');
?>
...
?>
  ...
  <tr id="dynamic_promotion2_photo_footer" style="display:none">
   <td colspan="5">&nbsp;</td>
  </tr>
...
<div style="float:left;"><a href="#" id="btn_add_promotion2_photo" class="button btn_add_promotion2_photo"  ><span><?php echo BTN_ADD_PHOTO;?></span></a></div>

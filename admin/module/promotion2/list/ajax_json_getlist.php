...
if( $_SESSION['already_login']){
	...
	if($_REQUEST['searching'] == '1'){
		...
		if($_REQUEST['search_disabled'] != '-1'){
			$search_condition .= " and promotion2.disabled = '".$_REQUEST['search_disabled']."' ";
		}
	...
	if($_REQUEST['list_action_now'] != ''){
		if($_REQUEST['list_action_now'] == '1' ){
			updateFieldDataFromTable(TB_PROMOTION2,"disabled",$record_id,1);
		}
		if($_REQUEST['list_action_now'] == '2' ){
			updateFieldDataFromTable(TB_PROMOTION2,"disabled",$record_id,0);
		}

		if($_REQUEST['list_action_now'] == '3' ){
			
			 deleteGroup(TB_PROMOTION2,$record_id,true,'');
		}
		generateHTACCESS();
	}
...
//	$return_data['table_list_content'] .= '<th width="100"><a href="#" class="orderby_title" orderby="display_date"><span>'.TITLE_DATE.'</span></a></th>';
	...
	// table content
	//////////////////////////////////////////////////////////
	$sql = "select promotion2.*,promotion2_desc.name as name from ".TB_PROMOTION2." as promotion2, ".TB_PROMOTION2_DESC." as promotion2_desc ";
	$sql .= " where ";
	$sql .= " promotion2.id = promotion2_desc.promotion2_id ";
	$sql .= " and promotion2.disabled <> '".DISABLED_DELETE."' ";
	$sql .= " and promotion2_desc.language_id = '".ADMIN_LANG_ID."'";
	$sql .= $search_condition;
	//$sql .= " group by promotion2.id  ";
...
//////////////////////////////////////////////////////////
	...
	$return_data['table_list_content'] .= getFirstPhoto(TB_PROMOTION2_PHOTO," promotion2_id='".$data['id']."' ","thumb",100,500);
